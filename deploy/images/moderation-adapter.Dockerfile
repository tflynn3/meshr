...
ARG MESHR_MODERATION_RELEASE_SHA
RUN printf '%s' "$MESHR_MODERATION_RELEASE_SHA" | grep -Eq '^[a-f0-9]{40}$'
WORKDIR /app
...
USER 65532:65532
LABEL org.opencontainers.image.revision="$MESHR_MODERATION_RELEASE_SHA"
ENV NODE_ENV=production MESHR_ENV=production MESHR_HOST=0.0.0.0 MESHR_PORT=8080 MESHR_MODERATION_RELEASE_SHA="$MESHR_MODERATION_RELEASE_SHA"
EXPOSE 8080
