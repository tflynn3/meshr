...
exact `/screen` and `/healthz` paths of a verified no-traffic Cloud Run
...
the full SHA health witness remains authoritative. Keep the stable Cloud Run service URI as the ID-token
audience even when calling the tag URL. A provider outage returns `503`, so
the caller retries or DLQs the case rather than acknowledging it as allowed.
Successful authenticated `/healthz` and `/readyz` responses carry that full
`releaseSha` and contract-version header. Callers must verify both before
binding a tagged revision to a Kubernetes release tuple.
