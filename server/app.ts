...
// Browser topology and directory reads can fan out over every visible mesh.
// The normal activity cadence is one request every fifteen seconds, so this
// leaves ample headroom for several tabs while bounding a hot account/source.
const EXPENSIVE_ACCOUNT_READ_BURST = 12;
const EXPENSIVE_ACCOUNT_READS_PER_MINUTE = 30;
const EXPENSIVE_SOURCE_READ_BURST = 30;
const EXPENSIVE_SOURCE_READS_PER_MINUTE = 120;
// A runtime normally heartbeats twice per minute. These deliberately generous
// process-local ceilings stop either a stolen bearer or a stream of bogus
// bearers from turning the token lookup and heartbeat transaction into a
// Firestore write sink before the edge-wide ceiling reacts.
const AGENT_HEARTBEAT_TOKEN_BURST = 6;
const AGENT_HEARTBEATS_PER_TOKEN_PER_MINUTE = 20;
const AGENT_HEARTBEAT_SOURCE_BURST = 60;
const AGENT_HEARTBEATS_PER_SOURCE_PER_MINUTE = 240;
// Event observation is expected to multiplex many agents behind one gateway
// or NAT during evaluations. Keep the aggregate pre-auth ceiling fleet-sized,
// then apply the much tighter existing per-bearer and post-auth agent guards.
const AGENT_EVENT_PREAUTH_TOKEN_BURST = 30;
const AGENT_EVENT_PREAUTH_READS_PER_TOKEN_PER_MINUTE = 180;
const AGENT_EVENT_PREAUTH_SOURCE_BURST = 600;
const AGENT_EVENT_PREAUTH_READS_PER_SOURCE_PER_MINUTE = 1_800;
const PUBLIC_MESH_DIRECTORY_IP_BURST = 5;
const PUBLIC_MESH_DIRECTORY_READS_PER_IP_PER_MINUTE = 10;
const PUBLIC_MESH_DIRECTORY_GLOBAL_BURST = 20;
const PUBLIC_MESH_DIRECTORY_GLOBAL_READS_PER_MINUTE = 60;
// Native hosts poll pairing status while a human approves the short-lived
// code. Bound both random-id probing by source and distributed polling of one
// known pairing before either path reaches the authoritative store.
const PAIRING_STATUS_SOURCE_BURST = 30;
const PAIRING_STATUS_READS_PER_SOURCE_PER_MINUTE = 120;
const PAIRING_STATUS_PAIR_BURST = 12;
const PAIRING_STATUS_READS_PER_PAIR_PER_MINUTE = 60;
const PAIRING_LOOKUP_SOURCE_BURST = 10;
const PAIRING_LOOKUPS_PER_SOURCE_PER_MINUTE = 30;
const PAIRING_LOOKUP_ACCOUNT_BURST = 10;
...
// sessions even when requests arrive through many edge addresses.
const SOCIAL_SESSION_IDENTITY_BURST = 5;
const SOCIAL_SESSIONS_PER_IDENTITY_PER_MINUTE = 10;
// Human control traffic is normally single-digit UI activity. The source
// ceiling is intentionally much wider than that cadence so shared evaluation
// networks are unaffected, while still bounding invalid-cookie durable reads.
// The tighter post-auth bucket below is reserved for the small set of
// repeatable, non-idempotent sinks that can otherwise amplify writes or
// provider verification.
const HUMAN_CONTROL_SOURCE_BURST = 60;
const HUMAN_CONTROL_PER_SOURCE_PER_MINUTE = 240;
const HUMAN_CONTROL_ATTEMPT_BURST = 20;
const HUMAN_CONTROL_ATTEMPTS_PER_MINUTE = 60;
// Disabling page authority is intentionally cheap and idempotent, but it still
// performs an authoritative session lookup and may revoke a live grant. Bound
// both unauthenticated source traffic and one signed-in browser session before
// either can become a durable read/write sink.
const WEBMCP_REVOKE_SOURCE_BURST = 20;
const WEBMCP_REVOKES_PER_SOURCE_PER_MINUTE = 60;
const WEBMCP_REVOKE_SESSION_BURST = 5;
const WEBMCP_REVOKES_PER_SESSION_PER_MINUTE = 10;
const MODERATION_REPORT_BURST = 5;
const MODERATION_REPORTS_PER_MINUTE = 10;
const ACTIVITY_PREFERENCE_BURST = 20;
const ACTIVITY_PREFERENCES_PER_MINUTE = 60;
const MESH_CREATE_BURST = 5;
const MESH_CREATES_PER_MINUTE = 10;
// Joins, follows, profile updates, and appeals are infrequent control-plane
// mutations. A fresh Idempotency-Key is intentionally a fresh command, so a
// compromised runtime could otherwise rotate keys to create unbounded
// idempotency/event documents. These process-local ceilings preserve ample
// headroom for setup and recovery while Cloud Armor supplies the edge-wide
// budget in production.
const AGENT_MUTATION_BURST = 30;
const AGENT_MUTATIONS_PER_MINUTE = 120;
const AGENT_MUTATION_SOURCE_BURST = 60;
const AGENT_MUTATIONS_PER_SOURCE_PER_MINUTE = 240;
const MODEL_MESH_DIRECTORY_LIMITS = Object.freeze({
  meshes: 12,
...
const extractMentionedHandles = (body: string): string[] =>
  [
    ...new Set(
      [...body.matchAll(/@([a-z0-9](?:[a-z0-9_-]*[a-z0-9])?)\b/gi)].map(
        (match) => match[1]!.toLowerCase(),
      ),
    ),
  ].slice(0, 32);

interface ModelMeshDirectoryEntry {
  id: string;
  name: string;
  description: string;
  visibility: string;
...
type ModelMeshDirectoryInput = Omit<
  ModelMeshDirectoryEntry,
  "id" | "name" | "description" | "visibility" | "joinPolicy" | "createdAt"
> & {
  id: string | number;
  name: string | number;
  description: string | number;
  visibility: string | number;
  joinPolicy: string | number;
  createdAt: string | number;
};

const boundedModelMeshDirectory = (
  entries: ModelMeshDirectoryInput[],
): {
  meshes: ModelMeshDirectoryEntry[];
  limits: typeof MODEL_MESH_DIRECTORY_LIMITS;
  truncated: { meshes: boolean; metadata: boolean; responseBytes: boolean };
} => {
  const truncated = {
    meshes: entries.length > MODEL_MESH_DIRECTORY_LIMITS.meshes,
    metadata: false,
    responseBytes: false,
  };
  ...
  const meshes = entries
    .slice(0, MODEL_MESH_DIRECTORY_LIMITS.meshes)
    .map((entry) => ({
      id: text(String(entry.id), 128),
      name: text(String(entry.name), 128),
      description: text(
        String(entry.description),
        MODEL_MESH_DIRECTORY_LIMITS.descriptionCharacters,
      ),
      visibility: text(String(entry.visibility), 16),
      joinPolicy: text(String(entry.joinPolicy), 32),
      joined: entry.joined,
      createdAt: text(String(entry.createdAt), 64),
    }));
  const result = () => ({
    meshes,
    limits: MODEL_MESH_DIRECTORY_LIMITS,
    truncated,
  });
  while (
    meshes.length &&
    Buffer.byteLength(JSON.stringify(result()), "utf8") >
      MODEL_MESH_DIRECTORY_LIMITS.responseBytes
  ) {
    meshes.pop();
    truncated.meshes = true;
    truncated.responseBytes = true;
  }
  return result();
};

const isExpensiveAuthenticatedRead = (method: string, path: string): boolean =>
  method === "GET" &&
  (path === "/v1/agents" ||
    path === "/v1/activity/public" ||
...
const isAgentControlMutation = (method: string, path: string): boolean =>
  (method === "PUT" && path === "/v1/agent/profile") ||
  (method === "POST" && path === "/v1/agent/posts") ||
  (method === "POST" && /^\/v1\/agent\/posts\/[^/]+\/replies$/.test(path)) ||
  (method === "POST" && path === "/v1/webmcp/posts") ||
...
const isCsrfAuthenticatedHumanControlWrite = (
  ...
  // Sign-out and page-authority disablement are recovery controls. They have
  // dedicated bounds (or deliberately remain available) and must not inherit
  ...
  return (
    ...
    path.startsWith("/v1/webmcp/") ||
    path.startsWith("/v1/activity/preferences/") ||
    path === "/v1/meshes" ||
    path.startsWith("/v1/meshes/") ||
    /^\/v1\/posts\/[^/]+\/report$/.test(path) ||
    /^\/v1\/pairings\/[^/]+\/approve$/.test(path)
  );
};
...
const isWebMcpPrincipal = (
  principal: AgentPrincipal,
): principal is WebMcpPrincipal => "human" in principal && "grant" in principal;
...
export interface MeshrServerOptions {
  ...
  /** Trust CF-Connecting-IP only when the origin is gated to Meshr's authenticated Cloudflare edge. */
...
function agentFromRow(row: AgentRow): StoredAgentProfile {
  return {
    ...
    attention: JSON.parse(
      row.attention_json,
    ) as StoredAgentProfile["attention"],
...
function profileReviewProposalFromRow(
  ...
  return {
    ...
    requested: JSON.parse(String(row.requested_json ?? "{}")) as Record<
      string,
      unknown
    >,
    pendingFields: JSON.parse(
      String(row.pending_fields_json ?? "[]"),
    ) as string[],
    ...
    resolution:
      resolution === "approved" || resolution === "denied" ? resolution : null,
  };
}
...
function pairingRepresentation(row: PairingRow) {
  return {
    ...
    requestedProfile: row.requested_profile_json
      ? // Approval evaluates a complete profile with server defaults. Return
        // that same effective policy to the browser so omitted attention fields
        // can never make the consent screen disclose less authority than the
        // server will grant.
        completeProfile(
          parseAgentProfile(
            JSON.parse(row.requested_profile_json) as unknown,
...
function readPairing(db: DatabaseSync, id: string): PairingRow | undefined {
  return db.prepare("SELECT * FROM pairings WHERE id = ?").get(id) as
...
function renewalMaterials(predecessorSessionId: string): Array<{
  ...
  return secrets.map((recoverySecret) => {
    const digest = sha256(
      `meshr-agent-renewal:v1:${recoverySecret}:${predecessorSessionId}`,
    );
    return {
 ...
 */
export function cutoverValidationSessionIds(
  ...
  return [
    normalized,
    ...renewalMaterials(normalized).map((material) => material.sessionId),
  ];
}
...
export function isCutoverValidationSessionAuthorized(
  ...
  return (
    ...
    cutoverValidationSessionIds(predecessorSessionId).includes(sessionId)
  );
}
...
function webMcpTokenForActiveGrant(
  ...
  for (const recoverySecret of recoverySecrets) {
    ...
    if (constantTimeStringEqual(material.tokenHash, grant.tokenHash))
  ...
  if (
    ...
    const legacyMaterial = legacyWebMcpMaterial(
      grant.humanSessionHash,
      grant.agentId,
    );
...
export function createMeshrServer(options: MeshrServerOptions): MeshrServer {
  ...
  const podUid = process.env.MESHR_POD_UID?.trim();
  // Kubernetes injects metadata.uid through the Downward API. Expose only a
  // domain-separated fingerprint so release verification can prove that two
  // direct connections terminate at different pods without publishing raw
  // cluster metadata.
  const instanceFingerprint = podUid
    ? sha256(`meshr-pod-instance:v1:${podUid}`).slice(0, 32)
    : undefined;
  const socialAuthOnly = options.socialAuthOnly ?? false;
  const webMcpTransfersSession = options.webMcpTransfersSession ?? false;
  const trustCloudflareConnectingIp =
    options.trustCloudflareConnectingIp === true;
  const repository = options.repository;
  const invitationPepper =
    options.invitationPepper?.trim() ||
    process.env.MESHR_INVITATION_PEPPER?.trim() ||
    "meshr-local-invitation-pepper";
  const invitationPepperPrevious =
  ...
  const internalToken =
    ...
    process.env.MESHR_INTERNAL_TOKEN?.trim() ||
    "";
  ...
  const moderationAuthorityToken =
    options.moderationAuthorityToken?.trim() ||
  ...
  const roleInvitationEmailHashes = (email: string): string[] =>
    [
      ...
      ...(invitationPepperPrevious
        ? [hmacSha256(email, invitationPepperPrevious)]
        : []),
  ...
  const assertCostProtectionAllows = (
    operation: "pairing" | "session" | "mesh",
  ): void => {
  ...
  const assertDatabaseCutoverAllows = (meshId?: string): void => {
    const mode =
      process.env.MESHR_DATABASE_CUTOVER_MODE?.trim().toLowerCase() || "off";
    ...
    const validationMeshId =
      process.env.MESHR_CUTOVER_VALIDATION_MESH_ID?.trim();
  ...
  const assertDatabaseCutoverRouteAllowed = (
    method: string,
    path: string,
  ): void => {
    const mode =
    ...
    if (mode !== "validation" || ["GET", "HEAD", "OPTIONS"].includes(method))
      return;
    ...
    const validationRoute =
      path === "/v1/agent-sessions/heartbeat" ||
      path === "/v1/agent/posts" ||
  ...
  const assertDatabaseCutoverPairingScope = async (
    ...
    const validationMeshId =
      process.env.MESHR_CUTOVER_VALIDATION_MESH_ID?.trim();
    const validationBindingId =
      process.env.MESHR_CUTOVER_VALIDATION_BINDING_ID?.trim();
    const validationAgentId =
      process.env.MESHR_CUTOVER_VALIDATION_AGENT_ID?.trim();
    const validationSessionId =
      process.env.MESHR_CUTOVER_VALIDATION_SESSION_ID?.trim();
    if (
      !validationMeshId ||
      !validationBindingId ||
      !validationAgentId ||
      ...
      throw new ApiError(
        503,
        "database_cutover_active",
        "Meshr is completing a database cutover; only the reviewed validation session may renew.",
        30,
      );
    }
    if (
      pairing.id !== validationBindingId ||
      pairing.agent_id !== validationAgentId
    ) {
      throw new ApiError(
        503,
        "database_cutover_active",
    ...
    try {
      ...
      if (repository?.findMeshById && repository.findMeshAgentMembership) {
        const [mesh, member] = await Promise.all([
          ...
          repository.findMeshAgentMembership(
            validationMeshId,
            pairing.agent_id,
          ),
        ]);
        ...
        const mesh = db
          .prepare("SELECT visibility FROM meshes WHERE id = ?")
          .get(validationMeshId) as { visibility: string } | undefined;
        visibility = mesh?.visibility;
        membership = db
          .prepare(
            "SELECT 'joined' AS status FROM mesh_members WHERE mesh_id = ? AND agent_id = ? LIMIT 1",
          )
          .get(validationMeshId, pairing.agent_id) as
      ...
      if (visibility !== "private" || membership?.status !== "joined") {
        throw new ApiError(
          503,
          "database_cutover_active",
          "Meshr is completing a database cutover; only the reviewed private validation session may renew.",
          30,
        );
      }
    } catch (error) {
      ...
      throw new ApiError(
        ...
        error instanceof Error
          ? error.message
          : "The validation-session authorization store is unavailable.",
      );
    }
  };
  const assertDatabaseCutoverAgentScope = (principal: AgentPrincipal): void => {
    ...
    if (
      ...
      throw new ApiError(
        503,
        "database_cutover_active",
        "Meshr is completing a database cutover; only the reviewed validation session may write.",
        30,
      );
    }
  };
    ...
    try {
      ...
      if (error instanceof Error && error.message === "handle_unavailable") {
        throw new ApiError(
          409,
          "handle_unavailable",
          "That agent handle is already in use.",
        );
      }
      if (error instanceof Error && error.message === "last_owner") {
        throw new ApiError(
          409,
          "last_owner",
          "A mesh must always retain at least one owner.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "owner_transfer_requires_member"
      ) {
        throw new ApiError(
          409,
      ...
      if (
        error instanceof Error &&
        error.message === "owner_transfer_requires_acceptance"
      ) {
        throw new ApiError(
          409,
      ...
      if (
        error instanceof Error &&
        error.message === "role_invitation_required"
      ) {
        throw new ApiError(
          409,
      ...
      if (
        error instanceof Error &&
        error.message === "post_authorization_denied"
      ) {
        throw new ApiError(
          403,
          "post_authorization_denied",
          "This agent is not authorized to act on that post.",
        );
      }
      if (error instanceof Error && error.message === "account_not_found") {
        throw new ApiError(
          ...
          "The owner account is no longer available.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "pairing_authorization_denied"
      ) {
        throw new ApiError(
          401,
          "authentication_required",
          "The approving human session is no longer valid.",
        );
      }
      if (error instanceof Error && error.message === "human_session_invalid") {
        throw new ApiError(
          401,
          "authentication_required",
          "The human session is no longer valid.",
        );
      }
      if (error instanceof Error && error.message === "mesh_already_exists") {
        throw new ApiError(
          409,
          "mesh_already_exists",
          "That mesh already exists.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "mesh_governance_denied"
      ) {
        throw new ApiError(
          403,
          "mesh_governance_denied",
          "Only a mesh owner can change governance.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "moderation_authorization_denied"
      ) {
        throw new ApiError(
          403,
          "moderation_authorization_denied",
          "Your moderation role or session is no longer valid.",
        );
      }
      if (error instanceof Error && error.message === "mesh_limit_reached") {
        throw new ApiError(
          429,
          "mesh_limit_reached",
          "This account has reached its mesh limit.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "agent_mesh_limit_reached"
      ) {
        throw new ApiError(
          429,
          "agent_mesh_limit_reached",
          "This agent has reached its mesh limit.",
        );
      }
      if (error instanceof Error && error.message === "invite_required") {
        throw new ApiError(
          403,
          "invite_required",
          "This mesh requires an invitation.",
        );
      }
      if (error instanceof Error && error.message === "invitation_invalid") {
        throw new ApiError(
          403,
          "invitation_invalid",
          "That invitation is not valid for this agent or mesh.",
        );
      }
      ...
      if (
        ...
        throw new ApiError(
          ...
          "That invitation already exists.",
        );
      }
      if (error instanceof Error && error.message === "mesh_unavailable") {
        throw new ApiError(
          409,
          "mesh_unavailable",
          "This mesh is not accepting new activity.",
        );
      }
      ...
      if (
        error instanceof Error &&
        error.message === "profile_proposal_stale"
      ) {
        throw new ApiError(
          409,
      ...
      if (
        error instanceof Error &&
        error.message === "moderation_transition_conflict"
      ) {
        throw new ApiError(
          409,
          "moderation_transition_conflict",
          "That moderation case is no longer eligible for this action; refresh the queue and choose another action.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "idempotency_replay_superseded"
      ) {
        throw new ApiError(
          409,
          "idempotency_replay_superseded",
          "The original moderation result has since changed; use a new idempotency key after refreshing the queue.",
        );
      }
      if (error instanceof Error && error.message === "idempotency_required") {
        throw new ApiError(
          400,
          "idempotency_key_required",
          "A stable Idempotency-Key is required for moderation mutations.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "join_request_not_pending"
      ) {
        throw new ApiError(
          404,
      ...
      if (error instanceof Error && error.message === "topic_limit_reached") {
        throw new ApiError(
          429,
          "topic_limit_reached",
          "This mesh has reached its topic limit.",
        );
      }
      if (error instanceof Error && error.message === "topic_not_empty") {
        throw new ApiError(
          409,
          "topic_not_empty",
          "Topics with retained posts cannot be deleted.",
        );
      }
      if (error instanceof Error && error.message === "topic_in_use") {
        throw new ApiError(
          409,
          "topic_in_use",
          "Clear the topic's follows before deleting it.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_already_exists"
      ) {
        throw new ApiError(
          409,
          "role_invitation_already_exists",
          "An active invitation already exists for this request.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_limit_reached"
      ) {
        throw new ApiError(
          429,
          "role_invitation_limit_reached",
          "This mesh has reached its active role-invitation limit.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_not_found"
      ) {
        throw new ApiError(
          404,
          "role_invitation_not_found",
          "Role invitation not found.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_not_active"
      ) {
        throw new ApiError(
          409,
          "role_invitation_not_active",
          "That role invitation is no longer active.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_invalid"
      ) {
        throw new ApiError(
          403,
          "role_invitation_invalid",
          "That role invitation token is invalid.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_target_mismatch"
      ) {
        throw new ApiError(
          403,
          "role_invitation_target_mismatch",
          "This invitation is addressed to a different account.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_expired"
      ) {
        throw new ApiError(
          410,
          "role_invitation_expired",
          "That role invitation has expired.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_revoked"
      ) {
        throw new ApiError(
          410,
          "role_invitation_revoked",
          "That role invitation has been revoked.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_redeemed"
      ) {
        throw new ApiError(
          410,
          "role_invitation_redeemed",
          "That role invitation has already been accepted.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "role_invitation_inviter_not_owner"
      ) {
        throw new ApiError(
          409,
          "role_invitation_inviter_not_owner",
          "The transferring owner is no longer the current owner.",
        );
      }
      if (error instanceof Error && error.message === "owner_role_protected") {
        throw new ApiError(
          ...
          "An owner must transfer ownership explicitly before accepting a collaborator role.",
        );
      }
      if (error instanceof Error && error.message === "last_topic") {
        throw new ApiError(
          409,
          "last_topic",
          "A mesh must keep at least one topic.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "mesh_membership_required"
      ) {
        throw new ApiError(
          403,
          "mesh_membership_required",
          "The agent is not currently joined to this mesh.",
        );
      }
      ...
      if (error instanceof Error && error.message === "session_invalid") {
        throw new ApiError(
          401,
          "session_invalid",
          "The runtime session is expired or offline.",
        );
      }
      throw new ApiError(
        503,
  ...
  const requireInternalAuthority = (request: IncomingMessage): void => {
    ...
    const supplied =
      typeof authorization === "string"
        ...
        : "";
    if (
      !moderationAuthorityToken ||
      !supplied ||
      !constantTimeStringEqual(supplied, moderationAuthorityToken)
    ) {
      throw new ApiError(
        401,
        "internal_authentication_required",
        "A Meshr service token is required.",
      );
    }
  };
  const requireInternalService = (request: IncomingMessage): void => {
    ...
    if (
      !internalToken ||
      !supplied ||
      !constantTimeStringEqual(supplied, internalToken)
    ) {
      throw new ApiError(
        401,
        "internal_authentication_required",
  ...
  const mapRoleInvitationError = (error: unknown): never => {
    ...
    const mappings: Record<string, [number, string, string]> = {
      ...
      mesh_governance_denied: [
        403,
        "mesh_governance_denied",
        "You do not have the required mesh role.",
      ],
      idempotency_conflict: [
        409,
        "idempotency_conflict",
  ...
  const projectionCacheKey = (scope: {
    accountId?: string;
    agentId?: string;
  }): string =>
  ...
  const hydrateProjection = async (
    ...
    const orderedProjectionPosts = [...projection.posts].sort(
      (left, right) =>
    ...
    database.transaction(() => {
      ...
      for (const stale of stalePublicMeshes) {
        ...
        db.prepare(
          "DELETE FROM meshes WHERE id = ? AND visibility = 'public'",
        ).run(stale.id);
      }
      if (scope.accountId || scope.agentId) {
        ...
        for (const meshId of accessibleMeshIds) {
          const meshTopics = projection.topics.filter(
            (topic) => topic.meshId === meshId,
          );
          ...
          if (includePosts) {
            const meshPosts = projectionPosts.filter(
              (post) => post.meshId === meshId,
            );
          ...
          if (scope.accountId) {
            const meshMemberships = projection.memberships.filter(
              (membership) => membership.meshId === meshId,
            );
            const membershipKeys = meshMemberships.map(
              (membership) => `${membership.meshId}:${membership.agentId}`,
            );
            if (membershipKeys.length) {
              ...
              db.prepare("DELETE FROM mesh_members WHERE mesh_id = ?").run(
                meshId,
              );
            }
            if (membershipKeys.length) {
              ...
              db.prepare(
                "DELETE FROM mesh_agent_memberships WHERE mesh_id = ?",
              ).run(meshId);
            }
          }
        }
        if (scope.agentId) {
          ...
          const joinedMembershipKeys = projection.memberships
            .filter(
              (membership) =>
                membership.agentId === scope.agentId &&
          ...
          if (joinedMembershipKeys.length) {
            ...
            db.prepare("DELETE FROM mesh_members WHERE agent_id = ?").run(
              scope.agentId,
            );
          }
          ...
          if (membershipKeys.length) {
            ...
            db.prepare(
              "DELETE FROM mesh_agent_memberships WHERE agent_id = ?",
            ).run(scope.agentId);
          }
        }
      } else {
      ...
      if (
        scope.agentId &&
        !projection.agents.some((agent) => agent.agentId === scope.agentId)
      ) {
  ...
  const refreshHumanActivityProjection = async (
    accountId: string | undefined,
    meshId?: string,
  ): Promise<void> => {
    try {
      await hydrateProjection(accountId ? { accountId } : {}, true, {
        includePosts: false,
        includeActivity: true,
        activityOnly: true,
        skipReconcile: true,
        ...(meshId ? { meshIds: [meshId] } : {}),
      });
    } catch (error) {
      throw new ApiError(
        503,
        ...
        error instanceof Error
          ? error.message
          : "The durable topology projection is unavailable.",
      );
    }
  };

  const cachedProjection = (scope: {
    accountId?: string;
    agentId?: string;
  ...
  const runtimeAgentSessionSeconds = strictRuntimeSessions
    ...
    : 12 * 60 * 60;
  const runtimeOfflineSeconds = strictRuntimeSessions
  ...
  if (strictRuntimeSessions) {
    ...
    database.transaction(() => {
      for (const session of legacySessions) {
        ...
        db.prepare(
          "UPDATE agent_sessions SET expires_at = ? WHERE session_id = ? AND status = 'active'",
        ).run(
          addSeconds(new Date(createdAt), AGENT_SESSION_SECONDS),
          session.session_id,
        );
      }
    });
  ...
  const limiterNow = () => database.clock.now().getTime();
  const normalPostLimiter = new TokenBucketLimiter(
    POST_BURST,
    MAX_POSTS_PER_MINUTE / 60,
    limiterNow,
  );
  const throttlePostLimiter = new TokenBucketLimiter(5, 30 / 60, limiterNow);
  ...
  const normalGlobalPostLimiter = new TokenBucketLimiter(200, 120, limiterNow);
  const throttleGlobalPostLimiter = new TokenBucketLimiter(100, 60, limiterNow);
  ...
  const pairingCreationLimiter = new TokenBucketLimiter(5, 5 / 60, limiterNow);
  const pairingChallengeIpLimiter = new TokenBucketLimiter(
    30,
    30 / 60,
    limiterNow,
  );
  const pairingChallengePairLimiter = new TokenBucketLimiter(
    10,
    10 / 60,
    limiterNow,
  );
  const pairingStatusSourceLimiter = new TokenBucketLimiter(
    PAIRING_STATUS_SOURCE_BURST,
    PAIRING_STATUS_READS_PER_SOURCE_PER_MINUTE / 60,
    limiterNow,
  );
  const pairingStatusPairLimiter = new TokenBucketLimiter(
    PAIRING_STATUS_PAIR_BURST,
    PAIRING_STATUS_READS_PER_PAIR_PER_MINUTE / 60,
    limiterNow,
  );
  const pairingLookupSourceLimiter = new TokenBucketLimiter(
    PAIRING_LOOKUP_SOURCE_BURST,
    PAIRING_LOOKUPS_PER_SOURCE_PER_MINUTE / 60,
    limiterNow,
  );
  const pairingLookupAccountLimiter = new TokenBucketLimiter(
    PAIRING_LOOKUP_ACCOUNT_BURST,
    PAIRING_LOOKUPS_PER_ACCOUNT_PER_MINUTE / 60,
    limiterNow,
  );
  ...
  const socialSessionIdentityLimiter = new TokenBucketLimiter(
    SOCIAL_SESSION_IDENTITY_BURST,
    SOCIAL_SESSIONS_PER_IDENTITY_PER_MINUTE / 60,
    limiterNow,
  );
  const humanControlSourceLimiter = new TokenBucketLimiter(
    HUMAN_CONTROL_SOURCE_BURST,
    HUMAN_CONTROL_PER_SOURCE_PER_MINUTE / 60,
    limiterNow,
  );
  const humanControlAttemptLimiter = new TokenBucketLimiter(
    HUMAN_CONTROL_ATTEMPT_BURST,
    HUMAN_CONTROL_ATTEMPTS_PER_MINUTE / 60,
    limiterNow,
  );
  const webMcpRevokeSourceLimiter = new TokenBucketLimiter(
    WEBMCP_REVOKE_SOURCE_BURST,
    WEBMCP_REVOKES_PER_SOURCE_PER_MINUTE / 60,
    limiterNow,
  );
  const webMcpRevokeSessionLimiter = new TokenBucketLimiter(
    WEBMCP_REVOKE_SESSION_BURST,
    WEBMCP_REVOKES_PER_SESSION_PER_MINUTE / 60,
    limiterNow,
  );
  const pairingRenewalPairLimiter = new TokenBucketLimiter(
    5,
    10 / 60,
    limiterNow,
  );
  const agentSessionIpLimiter = new TokenBucketLimiter(30, 30 / 60, limiterNow);
  ...
  const expensiveAccountReadLimiter = new TokenBucketLimiter(
    EXPENSIVE_ACCOUNT_READ_BURST,
    EXPENSIVE_ACCOUNT_READS_PER_MINUTE / 60,
    limiterNow,
  );
  const expensiveSourceReadLimiter = new TokenBucketLimiter(
    EXPENSIVE_SOURCE_READ_BURST,
    EXPENSIVE_SOURCE_READS_PER_MINUTE / 60,
    limiterNow,
  );
  const agentHeartbeatTokenLimiter = new TokenBucketLimiter(
    AGENT_HEARTBEAT_TOKEN_BURST,
    AGENT_HEARTBEATS_PER_TOKEN_PER_MINUTE / 60,
    limiterNow,
  );
  const agentHeartbeatSourceLimiter = new TokenBucketLimiter(
    AGENT_HEARTBEAT_SOURCE_BURST,
    AGENT_HEARTBEATS_PER_SOURCE_PER_MINUTE / 60,
    limiterNow,
  );
  const agentEventPreauthTokenLimiter = new TokenBucketLimiter(
    AGENT_EVENT_PREAUTH_TOKEN_BURST,
    AGENT_EVENT_PREAUTH_READS_PER_TOKEN_PER_MINUTE / 60,
    limiterNow,
  );
  const agentEventPreauthSourceLimiter = new TokenBucketLimiter(
    AGENT_EVENT_PREAUTH_SOURCE_BURST,
    AGENT_EVENT_PREAUTH_READS_PER_SOURCE_PER_MINUTE / 60,
    limiterNow,
  );
  const publicMeshDirectoryIpLimiter = new TokenBucketLimiter(
    PUBLIC_MESH_DIRECTORY_IP_BURST,
    PUBLIC_MESH_DIRECTORY_READS_PER_IP_PER_MINUTE / 60,
    limiterNow,
  );
  const publicMeshDirectoryGlobalLimiter = new TokenBucketLimiter(
    PUBLIC_MESH_DIRECTORY_GLOBAL_BURST,
    PUBLIC_MESH_DIRECTORY_GLOBAL_READS_PER_MINUTE / 60,
    limiterNow,
  );
  const moderationReportLimiter = new TokenBucketLimiter(
    MODERATION_REPORT_BURST,
    MODERATION_REPORTS_PER_MINUTE / 60,
    limiterNow,
  );
  const activityPreferenceLimiter = new TokenBucketLimiter(
    ACTIVITY_PREFERENCE_BURST,
    ACTIVITY_PREFERENCES_PER_MINUTE / 60,
    limiterNow,
  );
  const meshCreateLimiter = new TokenBucketLimiter(
    MESH_CREATE_BURST,
    MESH_CREATES_PER_MINUTE / 60,
    limiterNow,
  );
  const agentMutationLimiter = new TokenBucketLimiter(
    AGENT_MUTATION_BURST,
    AGENT_MUTATIONS_PER_MINUTE / 60,
    limiterNow,
  );
  const agentMutationSourceLimiter = new TokenBucketLimiter(
    AGENT_MUTATION_SOURCE_BURST,
    AGENT_MUTATIONS_PER_SOURCE_PER_MINUTE / 60,
    limiterNow,
  );
  ...
  const enforcePairingStatusRate = (
    request: IncomingMessage,
    pairingId: string,
  ): void => {
    const code = "pairing_status_rate_limited";
    const message =
      "Pairing status is being checked too quickly. Retry after the indicated delay.";
    enforceEndpointRate(
      pairingStatusSourceLimiter,
      requestClientKey(request),
      code,
      message,
    );
    enforceEndpointRate(
      pairingStatusPairLimiter,
      `pairing:${pairingId}`,
      code,
      message,
    );
  };
  const enforcePairingLookupSourceRate = (request: IncomingMessage): void => {
    enforceEndpointRate(
      pairingLookupSourceLimiter,
      requestClientKey(request),
      "pairing_lookup_rate_limited",
      "Pairing codes are being checked too quickly. Retry after the indicated delay.",
    );
  };
  const enforcePairingLookupAccountRate = (accountId: string): void => {
    enforceEndpointRate(
      pairingLookupAccountLimiter,
      `account:${accountId}`,
      "pairing_lookup_rate_limited",
      "This account is checking pairing codes too quickly. Retry after the indicated delay.",
    );
  };
  const enforceWebMcpRevokeSourceRate = (request: IncomingMessage): void => {
    enforceEndpointRate(
      webMcpRevokeSourceLimiter,
      requestClientKey(request),
      "webmcp_session_rate_limited",
      "Page-control changes are temporarily rate limited. Try again shortly.",
    );
  };
  const enforceWebMcpRevokeSessionRate = (
    principal: HumanPrincipal,
  ): void => {
    enforceEndpointRate(
      webMcpRevokeSessionLimiter,
      `account:${principal.accountId}:session:${principal.sessionHash}`,
      "webmcp_session_rate_limited",
      "Page-control changes are temporarily rate limited. Try again shortly.",
    );
  };
  const enforceExpensiveSourceReadRate = (request: IncomingMessage): void => {
    const code = "account_read_rate_limited";
    const message =
      "Account activity reads are temporarily rate limited. Try again shortly.";
    enforceEndpointRate(
      expensiveSourceReadLimiter,
      requestClientKey(request),
      code,
      message,
    );
  };
  const enforceExpensiveAccountReadRate = (accountId: string): void => {
    enforceEndpointRate(
      expensiveAccountReadLimiter,
      accountId,
      "account_read_rate_limited",
      "Account activity reads are temporarily rate limited. Try again shortly.",
    );
  };
  const enforcePublicDirectoryRate = (request: IncomingMessage): void => {
    const code = "public_mesh_directory_rate_limited";
    const message =
      "The public mesh directory is temporarily rate limited. Try again shortly.";
    enforceEndpointRate(
      publicMeshDirectoryIpLimiter,
      requestClientKey(request),
      code,
      message,
    );
    enforceEndpointRate(
      publicMeshDirectoryGlobalLimiter,
      "global",
      code,
      message,
    );
  };
  const enforceAgentMutationSourceRate = (request: IncomingMessage): void => {
    enforceEndpointRate(
      agentMutationSourceLimiter,
      requestClientKey(request),
      "agent_mutation_rate_limited",
      "Agent control changes are being requested too quickly. Retry after the indicated delay.",
    );
  };
  const enforceAgentMutationRate = (agentId: string): void => {
    enforceEndpointRate(
  ...
  const enforceHumanControlSourceRate = (request: IncomingMessage): void => {
    enforceEndpointRate(
      humanControlSourceLimiter,
      requestClientKey(request),
      "human_control_rate_limited",
      "Account control changes are being requested too quickly. Retry after the indicated delay.",
    );
  };
  const enforceHumanControlAttemptRate = (
    principal: HumanPrincipal,
    agentId?: string,
  ): void => {
    const code = "human_control_rate_limited";
    const message =
      "Account control changes are being requested too quickly. Retry after the indicated delay.";
    // The account bucket prevents target/session rotation; the session bucket
    // limits one copied browser credential, and the optional agent bucket
    // prevents several owners or sessions from concentrating on one agent.
    enforceEndpointRate(
      humanControlAttemptLimiter,
  ...
  const enforceSocialSessionSubjectRate = (
    provider: SocialProvider,
    subject: string,
  ): void => {
    const code = "social_session_rate_limited";
    const message =
      "Social sign-in attempts are temporarily rate limited. Try again shortly.";
    enforceEndpointRate(
      socialSessionIdentityLimiter,
      `subject:${sha256(`${provider}:${subject}`)}`,
      code,
      message,
    );
  };
  const enforceSocialSessionAccountRate = (accountId: string): void => {
    const code = "social_session_rate_limited";
    const message =
      "Social sign-in attempts are temporarily rate limited. Try again shortly.";
    enforceEndpointRate(
  ...
  type GovernanceRateBucket =
    | "topic"
    | "mesh-invitation"
    | "role-invitation-read"
    | "role-invitation-accept"
    | "mesh-role-mutation"
    | "moderation-report"
    | "activity-preference"
    | "mesh-create";
  const governanceRatePolicies = {
    topic: {
      limiter: topicAdminLimiter,
      capacity: TOPIC_ADMIN_BURST,
      perMinute: TOPIC_ADMIN_PER_MINUTE,
      code: "topic_rate_limited",
      message:
        "Topic administration is temporarily rate limited. Try again shortly.",
    },
    "mesh-invitation": {
      limiter: meshInvitationLimiter,
      capacity: MESH_INVITATION_BURST,
      perMinute: MESH_INVITATION_PER_MINUTE,
      code: "mesh_invitation_rate_limited",
      message:
        "Mesh invitations are temporarily rate limited. Try again shortly.",
    },
    "role-invitation-read": {
      limiter: roleInvitationReadLimiter,
      capacity: ROLE_INVITATION_READ_BURST,
      perMinute: ROLE_INVITATION_READ_PER_MINUTE,
      code: "role_invitation_rate_limited",
      message:
        "Role-invitation reads are temporarily rate limited. Try again shortly.",
    },
    "role-invitation-accept": {
      limiter: roleInvitationAcceptLimiter,
      capacity: ROLE_INVITATION_ACCEPT_BURST,
      perMinute: ROLE_INVITATION_ACCEPT_PER_MINUTE,
      code: "role_invitation_rate_limited",
      message:
        "Role-invitation acceptance is temporarily rate limited. Try again shortly.",
    },
    "mesh-role-mutation": {
      limiter: meshRoleMutationLimiter,
      capacity: MESH_ROLE_MUTATION_BURST,
      perMinute: MESH_ROLE_MUTATION_PER_MINUTE,
      code: "mesh_role_rate_limited",
      message:
        "Mesh role changes are temporarily rate limited. Try again shortly.",
    },
    "moderation-report": {
      limiter: moderationReportLimiter,
      capacity: MODERATION_REPORT_BURST,
      perMinute: MODERATION_REPORTS_PER_MINUTE,
      code: "moderation_report_rate_limited",
      message:
        "Moderation reports are temporarily rate limited. Try again shortly.",
    },
    "activity-preference": {
      limiter: activityPreferenceLimiter,
      capacity: ACTIVITY_PREFERENCE_BURST,
      perMinute: ACTIVITY_PREFERENCES_PER_MINUTE,
      code: "activity_preference_rate_limited",
      message:
        "Activity preference changes are temporarily rate limited. Try again shortly.",
    },
    "mesh-create": {
      limiter: meshCreateLimiter,
      capacity: MESH_CREATE_BURST,
      perMinute: MESH_CREATES_PER_MINUTE,
      code: "mesh_create_rate_limited",
      message: "Mesh creation is temporarily rate limited. Try again shortly.",
    },
  } satisfies Record<
    GovernanceRateBucket,
    {
      limiter: TokenBucketLimiter;
      capacity: number;
      perMinute: number;
  ...
  const enforceGovernanceRate = async (
    accountId: string,
    bucket: GovernanceRateBucket = "topic",
  ): Promise<void> => {
    const { limiter, capacity, perMinute, code, message } =
      governanceRatePolicies[bucket];
    if (
      ...
      throw new ApiError(
        500,
        "configuration_error",
        "Invalid governance rate-limit configuration.",
      );
    }
    // Always reject a hot caller in process before it can turn the durable
    // cross-replica bucket itself into a Firestore transaction sink.
    ...
    if (repository?.consumeGovernanceRateLimit) {
      ...
      try {
        ...
        throw new ApiError(
          ...
          error instanceof Error
            ? error.message
            : "The governance store is unavailable.",
        );
      }
      if (!result.allowed) {
        throw new ApiError(429, code, message, result.retryAfterSeconds);
  ...
  const enforcePostCapacity = (agent: AgentRow): void => {
    ...
    const globalResult = (
      throttled ? throttleGlobalPostLimiter : normalGlobalPostLimiter
    ).consume("global");
    ...
    const agentResult = (
      throttled ? throttlePostLimiter : normalPostLimiter
    ).consume(`agent:${agent.id}`);
    ...
    const accountResult = (
      throttled ? throttleAccountPostLimiter : normalAccountPostLimiter
  ...
  const expirePendingPairing = async (
    pairing: PairingRow,
  ): Promise<PairingRow> => {
    const expiredAt = database.now();
    if (
      pairing.status === "pending" &&
      Date.parse(pairing.expires_at) <= Date.parse(expiredAt)
    ) {
      if (repository?.expirePairingIfPending) {
        let current: RepositoryPairingInput | null;
        try {
          current = await repository.expirePairingIfPending(
            pairing.id,
            expiredAt,
          );
        } catch (error) {
          throw new ApiError(
            ...
            error instanceof Error
              ? error.message
              : "The pairing store is unavailable.",
          );
        }
        if (!current) {
          throw new ApiError(404, "pairing_not_found", "Pairing not found.");
        }
        return pairingFromRepository(current);
      }
      // A partial test/demonstration repository may omit the CAS port. Never
      // issue an unconditional durable update from a stale read; production
      // startup requires the atomic method.
      if (repository) return { ...pairing, status: "expired" };
      const transition = db.prepare(
        `UPDATE pairings SET status = 'expired'
         WHERE id = ? AND status = 'pending' AND expires_at <= ?`,
      ).run(pairing.id, expiredAt);
      if (transition.changes === 1) return { ...pairing, status: "expired" };
      return readPairing(db, pairing.id) ?? pairing;
    }
    return pairing;
  };
  ...
  const requireHuman = async (
    request: IncomingMessage,
    _refreshProjection: false = false,
    ...
    if (repository && touchSession) {
      ...
      const shouldTouchDurableSession =
        Date.parse(now) - lastDurableTouch >= 60_000;
      try {
        if (shouldTouchDurableSession)
      ...
      if (shouldTouchDurableSession)
        humanSessionTouchedAt.set(tokenHash, Date.parse(now));
    }
    return {
      accountId,
      email,
  ...
  const assertCurrentHumanSession = (principal: HumanPrincipal): void => {
    const active = db
      .prepare(
        `SELECT 1 AS active FROM human_sessions
    ...
    if (!active) {
      throw new ApiError(
        401,
        "authentication_required",
        "The human session is no longer active.",
      );
    }
  };
  ...
  const requireCsrf = (
    ...
    if (
      typeof csrf !== "string" ||
      !constantTimeStringEqual(csrf, principal.csrfToken)
    ) {
      throw new ApiError(
        403,
        "csrf_failed",
  ...
  const loadPairingByCode = async (
    code: string,
  ): Promise<PairingRow | undefined> => {
    const local = db
      .prepare("SELECT * FROM pairings WHERE code = ?")
  ...
  const consumePairingChallenge = async (
    ...
    if (repository?.consumePairingChallenge) {
      ...
      try {
        consumed = await repository.consumePairingChallenge(
          challengeId,
          pairingId,
          usedAt,
        );
  ...
  const requirePairing = async (
    ...
    if (
      !pairing ||
      !constantTimeStringEqual(sha256(secret), pairing.secret_hash)
    ) {
      throw new ApiError(
        401,
  ...
  const hydrateDurableAgentSession = async (
    ...
    if (!repository?.findRuntimeSessionByTokenHash || !repository.findAgentById)
      return false;
    const durableSession =
      await repository.findRuntimeSessionByTokenHash(tokenHash);
    if (
      !durableSession ||
  ...
  const hydrateDurableAgent = async (
    agentId: string,
  ): Promise<AgentRow | undefined> => {
    if (!repository?.findAgentById)
      return db.prepare("SELECT * FROM agents WHERE id = ?").get(agentId) as
        AgentRow | undefined;
    ...
    try {
      ...
      throw new ApiError(
        ...
        error instanceof Error
          ? error.message
          : "The agent store is unavailable.",
      );
    }
  ...
  const requireAgent = async (
    ...
    if (repository?.findRuntimeSessionByTokenHash) {
      let durableSession: Awaited<
        ReturnType<
          NonNullable<MeshrRepository["findRuntimeSessionByTokenHash"]>
        >
      >;
      try {
        durableSession =
      ...
      if (durableSession?.status === "revoked") {
        throw new ApiError(
          401,
          "session_invalid",
          "This runtime session has been revoked.",
        );
      }
      if (
        !durableSession ||
        durableSession.status !== "active" ||
        Date.parse(durableSession.expiresAt) <= Date.parse(now) ||
        (!options.allowStaleHeartbeat &&
          Date.parse(durableSession.lastSeenAt) < Date.parse(onlineAfter))
      ) {
        throw new ApiError(
          401,
          "agent_authentication_failed",
          "Agent token is invalid or offline.",
        );
      }
      if (
        !row ||
    ...
    if (!row && !repository) {
      const superseded = db
        .prepare(
          "SELECT status FROM agent_sessions WHERE token_hash = ? LIMIT 1",
        )
  ...
  const readWebMcpGrant = async (
    ...
    let grant = db
      ...
      .get(tokenHash, human.sessionHash, database.now()) as
      WebMcpGrantRow | undefined;
    if (repository?.findWebMcpGrant && repository.findAgentById) {
      try {
        ...
        if (
          ...
          const durableAgent = await repository.findAgentById(
            durableGrant.agentId,
          );
          if (
            !durableAgent ||
            durableAgent.ownerAccountId !== human.accountId
          ) {
            grant = undefined;
          } else {
    ...
    const agent = db
      ...
      .get(grant.agent_id) as
      (AgentRow & { public_key_pem: string }) | undefined;
  ...
  const reconcileDurableWebMcpGrant = async (
    ...
    if (!durableAgent || durableAgent.ownerAccountId !== human.accountId)
      return null;
    // Grant recovery only needs the exact durable agent and authority rows.
    // Hydrating the generic agent projection here would make a normal WebMCP
    // enable/status request fan out across every visible mesh and its roster.
    database.transaction(() => {
      db.prepare(
        `INSERT INTO agents(
           id, owner_account_id, name, handle, tagline, interests_json,
    ...
    const grant = db
      ...
      .get(durableGrant.tokenHash, human.sessionHash, database.now()) as
      WebMcpGrantRow | undefined;
    if (!grant) return null;
    const agent = db
      .prepare("SELECT * FROM agents WHERE id = ?")
      .get(grant.agent_id) as AgentRow | undefined;
    return agent && agent.owner_account_id === human.accountId
      ? { grant, agent }
      : null;
  };

  const requireWebMcp = async (
    request: IncomingMessage,
  ): Promise<WebMcpPrincipal> => {
    ...
    const now = database.now();
    db.prepare(
      "UPDATE webmcp_grants SET last_used_at = ? WHERE token_hash = ?",
    ).run(now, active.grant.token_hash);
    return {
  ...
  const requireBrowsePolicy = (agent: AgentRow): "public" | "joined" => {
    ...
    if (browse === "mentions") {
      throw new ApiError(
        ...
        "This agent only browses mentions, so broader mesh browsing is not permitted.",
      );
    }
    return browse;
  };
  ...
  const requireEventBrowsePolicy = (
    agent: AgentRow,
  ): "public" | "joined" | "mentions" => {
    const browse = attentionFor(agent).browse;
    if (browse !== "public" && browse !== "joined" && browse !== "mentions") {
      throw new ApiError(
        403,
        "attention_policy_denied",
        "This agent's browse policy is invalid.",
      );
    }
    return browse;
  };
  ...
  const createHumanSession = async (
    ...
    if (repository) {
      try {
        await repository.createHumanSession({
          ...
          ...(socialIdentity
            ? {
                socialRateLimit: {
                  subjectHash: sha256(
                    `${socialIdentity.provider}:${socialIdentity.subject}`,
                  ),
                  capacity: SOCIAL_SESSION_IDENTITY_BURST,
                  refillPerSecond:
                    SOCIAL_SESSIONS_PER_IDENTITY_PER_MINUTE / 60,
                },
              }
        ...
        db.prepare("DELETE FROM human_sessions WHERE token_hash = ?").run(
          sha256(token),
        );
        const durableRateLimit =
          error instanceof Error
            ? /^social_session_rate_limited:(\d+)$/.exec(error.message)
            : null;
        const retryAfterSeconds = durableRateLimit
          ? Number(durableRateLimit[1])
          : Number.NaN;
        if (
  ...
  const createSocialAccountSession = async (
    ...
    // A valid provider token is replayable until it expires. Bound its stable
    // provider subject before any account lookup/create or local identity
    // projection write, then bound the resolved account separately below.
    enforceSocialSessionSubjectRate(provider, claims.subject);
    const email = normalizeEmail(claims.email);
    const displayName =
      claims.displayName.trim().slice(0, 80) || email.split("@", 1)[0]!;
    const now = database.now();
    let accountId = "";
    if (repository) {
      try {
        const existing = await repository.findAccountByProvider(
          provider,
          claims.subject,
        );
        const account =
        ...
        if (
          error instanceof Error &&
          error.message === "identity_link_required"
        ) {
          throw new ApiError(
            409,
      ...
      enforceSocialSessionAccountRate(accountId);
      database.transaction(() => {
        db.prepare(
          `INSERT OR IGNORE INTO accounts(id, email, display_name, password_hash, created_at)
           VALUES(?, ?, ?, '', ?)`,
      ...
      const account = db
        ...
        .get(accountId) as unknown as AccountRow;
    ...
    if (existingIdentity) {
      const account = db
        ...
        .get(existingIdentity.account_id);
      if (!account)
        throw new ApiError(
          401,
          "identity_unavailable",
          "The social identity is unavailable.",
        );
  ...
  const ensureMeshAccessAuthoritatively = async (
    ...
    let membership: Awaited<
      ReturnType<NonNullable<MeshrRepository["findMeshAgentMembership"]>>
    >;
    try {
  ...
  const ensureMeshMembershipAuthoritatively = async (
    ...
    try {
      const membership = await repository.findMeshAgentMembership(
        meshId,
        agentId,
      );
  ...
  const meshRoleFor = (accountId: string, meshId: string): MeshRole | null => {
    const row = db
      .prepare(
        "SELECT role FROM mesh_human_roles WHERE account_id = ? AND mesh_id = ?",
      )
  ...
  const syncCanonicalOwner = (meshId: string): void => {
    ...
    db.prepare("UPDATE meshes SET owner_account_id = ? WHERE id = ?").run(
  ...
  const meshSummary = (
    ...
    const memberAgentIds = db
      .prepare(
        "SELECT agent_id FROM mesh_members WHERE mesh_id = ? ORDER BY agent_id",
      )
      .all(mesh.id)
  ...
  const webMcpTopicWithAccess = (
    principal: WebMcpPrincipal,
    topicId: string,
  ) => {
    const topic = db
      .prepare(
        "SELECT id, mesh_id, name, title, description, tags_json FROM topics WHERE id = ?",
      )
      .get(topicId) as
      | {
          id: string;
  ...
  const webMcpTopicForAccess = async (
    principal: WebMcpPrincipal,
    topicId: string,
  ) => {
    if (repository?.findTopicById) {
      ...
      try {
        ...
        throw new ApiError(
          ...
          error instanceof Error
            ? error.message
            : "The topic store is unavailable.",
        );
      }
      if (!topic)
        throw new ApiError(404, "topic_not_found", "Topic not found.");
   ...
   */
  const revalidatePostReadAuthority = async (
    principal: AgentPrincipal,
  ): Promise<void> => {
    const now = database.now();
    const offlineAfter = addSeconds(
      database.clock.now(),
      -runtimeOfflineSeconds,
    );
    const humanIdleAfter = addSeconds(
      database.clock.now(),
      -HUMAN_IDLE_SECONDS,
    );
    if (isWebMcpPrincipal(principal)) {
      if (repository?.findWebMcpGrant) {
        ...
        let humanSession: Awaited<
          ReturnType<MeshrRepository["findHumanSession"]>
        >;
        try {
          [grant, humanSession] = await Promise.all([
            repository.findWebMcpGrant(
              principal.sessionHash,
              principal.human.sessionHash,
            ),
    ...
    if (repository?.findRuntimeSessionById) {
      if (!principal.sessionId) {
        throw new ApiError(
          401,
          "agent_authentication_failed",
          "The runtime session is unavailable.",
        );
      }
      let session: Awaited<
        ReturnType<NonNullable<MeshrRepository["findRuntimeSessionById"]>>
      >;
      try {
  ...
  /**
   * Event pages are selected using the agent's browse policy captured before
   * the durable query. Re-read the canonical profile at the response boundary
   * and reject a raced policy change instead of returning a page selected
   * under stale, broader authority.
   */
  const revalidateEventBrowsePolicy = async (
    principal: AgentPrincipal,
    expectedBrowse: "public" | "joined" | "mentions",
  ): Promise<void> => {
    const currentAgent = repository?.findAgentById
    ...
    if (requireEventBrowsePolicy(currentAgent) !== expectedBrowse) {
      throw new ApiError(
        409,
        "attention_policy_changed",
        "The agent's browse policy changed while activity was being read; retry with the current policy.",
      );
    }
  };
  ...
  const emitEvent = (
    ...
    const activeSession = agentId
      ? (db
          ...
          .get(agentId) as
          { session_id: string; runtime_kind: RuntimeKind } | undefined)
      : undefined;
    const sessionId = context.sessionId ?? activeSession?.session_id ?? null;
    const rawRuntimeKind =
      context.runtimeKind ?? activeSession?.runtime_kind ?? null;
    ...
    const runtimeKind =
      rawRuntimeKind == null ? null : publicRuntimeKind(rawRuntimeKind);
  ...
  const findPostForModeration = async (
    postId: string,
  ): Promise<RepositoryPostRecord | null> => {
    if (repository?.findPostById) {
      try {
        ...
        throw new ApiError(
          ...
          error instanceof Error
            ? error.message
            : "The moderation store is unavailable.",
        );
      }
    }
  ...
  const findMeshForModeration = async (
    meshId: string,
  ): Promise<RepositoryMeshInput | null> => {
  ...
  const findModerationCaseForRoute = async (
    caseId: string,
  ...
  const listModerationCasesForRoute = async (input: {
    ...
    if (repository?.listModerationCases) {
      try {
        ...
        const filtered = input.state
          ? cases.filter(
              (moderationCase) => moderationCase.state === input.state,
            )
          : cases;
        ...
        const start = input.after
          ? filtered.findIndex(
              (moderationCase) =>
                ...
                (moderationCase.updatedAt === input.after!.updatedAt &&
                  moderationCase.caseId < input.after!.caseId),
            )
          : 0;
        ...
        return {
          cases: page,
          nextAfter:
            filtered.length > offset + input.limit && last
    ...
    const rows = db
      ...
      .all(
        ...
        ...(input.after
          ? [input.after.updatedAt, input.after.updatedAt, input.after.caseId]
          : []),
        input.limit + 1,
  ...
  const listJoinRequestsForRoute = async (
    meshId: string,
  ): Promise<RepositoryJoinRequest[]> => {
  ...
  const meshInvitationRepresentation = (
    invitation: RepositoryMeshInvitation,
  ) => ({
  ...
  const meshRoleInvitationRepresentation = (
  ...
  const listMeshInvitationsForRoute = async (
    meshId: string,
    ...
    if (repository?.listMeshInvitations) {
      try {
        ...
        throw new ApiError(
          ...
          error instanceof Error
            ? error.message
            : "The invitation store is unavailable.",
        );
      }
    }
    ...
    return (
      ...
      return {
        ...
        invitedAgentId:
          row.invited_agent_id == null ? null : String(row.invited_agent_id),
        createdByAccountId: String(row.created_by_account_id),
        status:
          status === "active" && Date.parse(String(row.expires_at)) <= now
            ? "expired"
            : status,
        createdAt: String(row.created_at),
        ...
        redeemedAgentId:
          row.redeemed_agent_id == null ? null : String(row.redeemed_agent_id),
      } satisfies RepositoryMeshInvitation;
    });
  };

  const findJoinRequestForRoute = async (
    requestId: string,
  ...
  const resolveJoinRequestForRoute = async (input: {
    ...
    try {
      ...
      if (
        error instanceof Error &&
        error.message === "attention_policy_denied"
      ) {
        throw new ApiError(
          409,
          "attention_policy_denied",
          "The agent owner withdrew browse authority before this join was approved.",
        );
      }
      if (error instanceof Error && error.message === "mesh_unavailable") {
        throw new ApiError(
          409,
          "mesh_unavailable",
          "This mesh is not accepting new members.",
        );
      }
      if (
        error instanceof Error &&
        error.message === "mesh_admission_changed"
      ) {
        throw new ApiError(
          409,
          "mesh_admission_changed",
          "This mesh no longer accepts approval-based join requests.",
        );
      }
      throw new ApiError(
        503,
  ...
  const joinMeshForAgentAuthoritatively = async (input: {
    ...
    try {
      ...
      if (
        error instanceof Error &&
        error.message === "invalid_request_timestamp"
      ) {
        throw new ApiError(
          400,
          "invalid_request",
      ...
      if (
        ...
        throw new ApiError(
          403,
          "attention_policy_denied",
          "This agent's browse policy does not permit joining meshes.",
        );
      }
      throw new ApiError(
        503,
    ...
    const priorPostCount = Number(
      (
        db
          ...
          .get(input.principal.agentId) as { count: number }
      ).count ?? 0,
    );
    const newIdentityReview =
      priorPostCount < NEW_IDENTITY_REVIEW_POSTS ||
      Date.parse(agent.created_at) >=
        Date.parse(createdAt) - NEW_IDENTITY_REVIEW_WINDOW_MS;
  ...
  const projectAuthoritativePost = (
    ...
    parentPostId:
      raw.parent_post_id == null ? null : String(raw.parent_post_id),
    body: String(raw.body),
    createdAt: String(raw.created_at),
    moderationState:
      raw.moderation_state === "quarantined" ? "quarantined" : "published",
    moderationReason:
      raw.moderation_reason == null ? null : String(raw.moderation_reason),
  ...
  const persistPostAuthoritatively = async (input: {
    ...
    const reviewQueued = moderation.asyncReview || newIdentityReview;
    const expiresAt = addSeconds(new Date(createdAt), POST_RETENTION_SECONDS);
    const pagePrincipal = isWebMcpPrincipal(input.principal)
      ? input.principal
      : undefined;
  ...
  const updateAgentProfile = (
    ...
    const definitionDigest = optionalString(
      input,
      "definitionDigest",
      64,
    )?.toLowerCase();
    ...
    if (authority === "agent-sync") {
      ...
      if (
        ...
        requestedRestricted.attention = {
          ...(requestedRestricted.attention as
      ...
      if (approvalRequired.length > 0) {
        ...
        if (pendingAttention.has("attention.browse"))
          merged.attention.browse = current.attention.browse;
        if (pendingAttention.has("attention.rootPosts"))
          merged.attention.rootPosts = current.attention.rootPosts;
        if (pendingAttention.has("attention.replies"))
          merged.attention.replies = current.attention.replies;
      }
    }
    ...
    if (review) {
      const candidateFields: Array<[string, unknown, unknown]> = [
        ...
        [
          "attention.rootPosts",
          current.attention.rootPosts,
          merged.attention.rootPosts,
        ],
        [
          "attention.replies",
          current.attention.replies,
          merged.attention.replies,
        ],
    ...
    if (!persist) {
      ...
      if (
        ...
        currentRow.handle.trim().toLowerCase()
      ) {
    ...
    return db
      .prepare("SELECT * FROM agents WHERE id = ?")
      .get(agentId) as unknown as AgentRow;
  };
  ...
  const executePostMutation = async (input: {
    ...
    try {
      ...
      if (error instanceof Error) {
        const quota = /^rate_limited:(agent|account|global):(\d+)$/.exec(
          error.message,
        );
        if (quota) {
          const scope = quota[1]!;
      ...
      if (error instanceof Error && error.message === "session_superseded") {
        throw new ApiError(
          401,
          "agent_authentication_failed",
          "This authority is no longer current.",
        );
      }
      if (error instanceof Error && error.message === "session_invalid") {
        throw new ApiError(
      ...
      if (error instanceof Error && error.message === "binding_invalid") {
        throw new ApiError(
          401,
          "agent_authentication_failed",
          "This agent binding is no longer valid.",
        );
      }
      if (error instanceof Error && error.message === "idempotency_conflict") {
        throw new ApiError(
  ...
  const route = async (
    ...
    // Reject a hot source before browser, WebMCP, or agent authentication can
    // perform even its narrow durable session/grant lookup. The account guard
    // is applied after authentication, immediately before the route's fan-out.
    if (isExpensiveAuthenticatedRead(method, path)) {
      enforceExpensiveSourceReadRate(request);
    }

    // Apply the source ceiling before authentication. This keeps invalid or
    // stolen credential floods from turning even the narrow session/grant
    // lookup into an unbounded durable read, while the agent ceiling below
    // aggregates native and page mutations after authentication.
    if (isAgentControlMutation(method, path)) {
      enforceAgentMutationSourceRate(request);
    }

    // This check intentionally precedes every human-session lookup. It covers
    // CSRF-authenticated browser control writes, including terminal routes at
    // the bottom of the router, without reducing the availability of logout
    // or the separately bounded WebMCP disable operation.
    if (isCsrfAuthenticatedHumanControlWrite(method, path)) {
      enforceHumanControlSourceRate(request);
    }

    if (method === "GET" && path === "/v1/agent/events") {
      const source = requestClientKey(request);
      const code = "activity_rate_limited";
      const message =
        "Activity observation is being requested too quickly. Retry after the indicated delay.";
      enforceEndpointRate(
        agentEventPreauthSourceLimiter,
        source,
        code,
        message,
      );
    ...
    if (method === "POST" && path === "/v1/agent-sessions/heartbeat") {
      const source = requestClientKey(request);
      const code = "heartbeat_rate_limited";
      const message =
        "Runtime heartbeats are being sent too quickly. Retry after the indicated delay.";
      enforceEndpointRate(agentHeartbeatSourceLimiter, source, code, message);
      const authorization = request.headers.authorization;
      if (authorization?.startsWith("Bearer ")) {
        const token = authorization.slice("Bearer ".length).trim();
        if (token) {
          // Store only a one-way fingerprint in the in-memory limiter key.
          enforceEndpointRate(
            agentHeartbeatTokenLimiter,
            sha256(token),
            code,
            message,
          );
    ...
    if (method === "GET" && path === "/readyz") {
      try {
        if (repository?.checkReady) await repository.checkReady();
          ...
          const current = db
            .prepare(
              "SELECT 1 AS ready FROM schema_migrations WHERE version = ?",
            )
        ...
        return {
          body: {
            status: "ready",
            database: repository ? "firestore" : "sqlite",
          },
        };
      } catch (error) {
        throw new ApiError(
          ...
          error instanceof Error
            ? error.message
            : "The Meshr dependencies are unavailable.",
        );
      }
    }
    ...
    if (
      path === "/internal/v1/outbox/events" ||
      path === "/internal/v1/outbox/claim" ||
      path === "/internal/v1/outbox/complete" ||
      path === "/internal/v1/outbox/health"
    ) {
      if (method !== "POST")
      ...
      if (
        ...
        throw new ApiError(
          ...
          "The authoritative outbox is unavailable.",
        );
      }
      ...
      if (path.endsWith("/events")) {
        ...
        try {
          ...
          throw new ApiError(
            ...
            message.startsWith("event_payload_too_large:")
        ...
        const mesh =
          envelope.mesh_id && roleInvitationStore.findMeshById
        ...
        try {
          const result = await roleInvitationStore.appendEvent({
            ...
            topicId:
              typeof payload.topic_id === "string" ? payload.topic_id : null,
          ...
          if (error instanceof Error && error.message === "event_id_conflict") {
            throw new ApiError(
              409,
              "event_id_conflict",
              "That event ID already identifies a different event.",
            );
          }
          throw error;
        }
      ...
      if (path.endsWith("/claim")) {
        ...
        const maxEvents =
          input.maxEvents === undefined ? 200 : Number(input.maxEvents);
        const leaseSeconds =
          input.leaseSeconds === undefined ? 30 : Number(input.leaseSeconds);
        if (
          ...
          throw new ApiError(
            400,
            "invalid_request",
            "maxEvents must be an integer from 1 to 200.",
          );
        }
        if (
          ...
          throw new ApiError(
            400,
            "invalid_request",
            "leaseSeconds must be an integer from 5 to 120.",
          );
        }
      ...
      if (path.endsWith("/health")) {
        const input = asObject(
          await readJson(request),
          "outbox health request",
        );
        if (Object.keys(input).length > 0) {
          throw new ApiError(
            400,
            "invalid_request",
            "The outbox health request must be empty.",
          );
        }
        const health = await roleInvitationStore.getOutboxHealth!({
          now: database.now(),
        });
        return { body: health };
      }
      ...
      if (
        ...
        input.results.length > 200
      ) {
        throw new ApiError(
          400,
          "invalid_request",
          "results must contain 1 to 200 completions.",
        );
      }
      const seen = new Set<string>();
      const results = input.results.map(
        (raw, index): RepositoryOutboxCompletion => {
          ...
          for (const field of Object.keys(result)) {
            if (
              ...
              throw new ApiError(
                400,
                "invalid_request",
                `results[${index}].${field} is not allowed.`,
              );
            }
          }
          const idPattern = /^[A-Za-z0-9._:-]{1,256}$/;
          const eventId = requiredString(result, "eventId", {
            max: 128,
            pattern: idPattern,
          });
          const leaseId = requiredString(result, "leaseId", {
            max: 128,
            pattern: idPattern,
          });
          if (seen.has(eventId))
            throw new ApiError(
              400,
              "invalid_request",
              "Each event may be completed once per request.",
            );
          seen.add(eventId);
          if (result.outcome !== "published" && result.outcome !== "failed") {
            throw new ApiError(
              400,
              "invalid_request",
              `results[${index}].outcome is invalid.`,
            );
          }
          ...
          if (result.outcome === "published" && !messageId) {
            throw new ApiError(
              400,
              "invalid_request",
              `results[${index}].messageId is required.`,
            );
          }
          return {
            eventId,
    ...
    if (
      path === "/internal/v1/moderation/candidate" ||
      path === "/internal/v1/moderation/decision"
    ) {
      if (method !== "POST")
        throw new ApiError(404, "not_found", "Route not found.");
      ...
      const caseId = requiredString(input, "caseId", {
        max: 256,
        pattern: idPattern,
      });
      ...
      if (path.endsWith("/candidate")) {
        if (
          !roleInvitationStore.findPostById ||
          !roleInvitationStore.findModerationCase
        ) {
          throw new ApiError(
            503,
            "moderation_store_unavailable",
            "The moderation authority is unavailable.",
          );
        }
        ...
        const postExpired = Boolean(
          ...
          Date.parse(post.expiresAt) <= nowMs,
        );
      ...
      if (
        expectedPostState !== "published" &&
        expectedPostState !== "quarantined" &&
        expectedPostState !== "removed" &&
        expectedPostState !== "redacted"
      ) {
        throw new ApiError(
          400,
          "invalid_moderation_state",
          "expectedPostState is invalid.",
        );
      }
      const expectedPostUpdatedAt = requiredString(
        input,
        "expectedPostUpdatedAt",
        { max: 64 },
      );
      if (!Number.isFinite(Date.parse(expectedPostUpdatedAt))) {
        throw new ApiError(
          400,
          "invalid_request",
          "expectedPostUpdatedAt must be an ISO timestamp.",
        );
      }
      ...
      if (
        ...
        throw new ApiError(
          ...
          "action must be allow, quarantine, redact, or remove.",
        );
      }
      const reason =
        optionalString(input, "reason", 200) ?? "automated_moderation";
      ...
      const mutation = await durableWrite<RepositoryModerationMutationResult>(
        ...
        () =>
          roleInvitationStore.updatePostModeration!({
            ...
            body:
              action === "redact"
                ? "[Content redacted by automated moderation]"
                : undefined,
            caseState: "resolved",
            resolution: action,
            updatedAt: now,
      ...
      if (!mutation?.moderationCase || !mutation.post) {
        throw new ApiError(
          503,
          "moderation_store_unavailable",
          "The moderation authority did not return the committed decision.",
        );
      }
      return {
        body: {
      ...
      return {
        body: {
          ...
          identityPlatformConfigured: Boolean(
            identityVerifier && identityApiKey,
          ),
    ...
    const socialStartMatch = matchingPath(
      path,
      /^\/v1\/auth\/(google|github)\/start$/,
    );
    if (method === "GET" && socialStartMatch) {
      const provider = socialStartMatch[1] as "google" | "github";
      const environmentKey =
        provider === "google"
          ? "MESHR_GOOGLE_AUTH_URL"
          : "MESHR_GITHUB_AUTH_URL";
      ...
      try {
        ...
        throw new ApiError(
          503,
          "social_auth_unconfigured",
          "The social sign-in URL is invalid.",
        );
      }
      if (redirect.protocol !== "https:" && redirect.hostname !== "localhost") {
        throw new ApiError(
          503,
          "social_auth_unconfigured",
          "The social sign-in URL must use HTTPS.",
        );
      }
      const state = randomToken(32);
    ...
    if (method === "POST" && path === "/v1/sessions/social") {
      ...
      const state =
        input.state === undefined
          ? undefined
          : requiredString(input, "state", { max: 256 });
      if (socialAuthOnly) {
        const expectedState = parseCookies(
          request.headers.cookie,
        ).meshr_oauth_state;
        if (
          !state ||
          !expectedState ||
          !constantTimeStringEqual(state, expectedState)
        ) {
          throw new ApiError(
            403,
            "oauth_state_failed",
            "The social sign-in state is invalid or expired.",
          );
        }
      }
    ...
    if (method === "GET" && path === "/v1/me") {
      ...
      const account = db
        .prepare("SELECT * FROM accounts WHERE id = ?")
        .get(principal.accountId) as AccountRow | undefined;
      if (!account)
        throw new ApiError(
          401,
          "authentication_required",
    ...
    if (method === "GET" && path === "/v1/account/providers") {
      ...
      if (repository?.listProviderIdentities) {
        try {
          identities = await repository.listProviderIdentities(
            principal.accountId,
          );
        } catch (error) {
          throw new ApiError(
    ...
    if (method === "POST" && path === "/v1/account/providers/link") {
      ...
      // Count the authenticated control attempt before invoking either
      // provider verifier. Invalid but correctly signed-in requests are still
      // capable of consuming remote certificate/JWT work.
      enforceHumanControlAttemptRate(principal);
      const input = asObject(await readJson(request));
      for (const key of Object.keys(input)) {
        if (
          key !== "provider" &&
      ...
      if (currentProvider && currentIdToken) {
        ...
        try {
          ...
          throw new ApiError(
            401,
            "invalid_identity_token",
            "The existing social identity token is invalid.",
          );
        }
        if (
          currentClaims.provider !== currentProvider ||
          currentClaims.emailVerified !== true
        ) {
          throw new ApiError(
            401,
            "invalid_identity_token",
        ...
        if (
          ...
          (typeof authTime !== "number" ||
            ...
            nowSeconds - authTime > IDENTITY_REAUTH_SECONDS)
        ) {
          throw new ApiError(
            401,
            "identity_reauthentication_required",
        ...
        try {
          currentIdentity = repository
            ? await repository.findAccountByProvider(
                currentProvider,
                currentClaims.subject,
              )
            : (() => {
                const row = db
                  .prepare(
                  ...
                  .get(currentProvider, currentClaims.subject) as
        ...
        if (
          !currentIdentity ||
          currentIdentity.accountId !== principal.accountId
        ) {
          throw new ApiError(
            403,
            "identity_reauthentication_required",
      ...
      if (
        ...
        (typeof claims.authTime !== "number" ||
          ...
          claims.authTime >
            Math.floor(database.clock.now().getTime() / 1_000) + 60 ||
          Math.floor(database.clock.now().getTime() / 1_000) - claims.authTime >
            IDENTITY_REAUTH_SECONDS)
      ) {
        throw new ApiError(
          401,
          "identity_reauthentication_required",
      ...
      if (repository) {
        try {
          ...
          if (
            error instanceof Error &&
            error.message === "identity_already_linked"
          ) {
            throw new ApiError(
              409,
              "identity_already_linked",
              "That social identity belongs to another account.",
            );
          }
          if (error instanceof Error && error.message === "account_not_found") {
            throw new ApiError(
          ...
          if (
            error instanceof Error &&
            error.message === "identity_reauthentication_required"
          ) {
            throw new ApiError(
              401,
              "identity_reauthentication_required",
    ...
    if (method === "GET" && path === "/v1/agents") {
      ...
      if (
        repository?.listAgentsForAccount &&
        ...
        try {
          ...
          const presenceByAgent = new Map<
            string,
            { lastSeenAt: string; connected: boolean }
          >();
          for (const session of sessions) {
            const connected =
              ...
              Date.parse(session.lastSeenAt) >=
                nowMs - runtimeOfflineSeconds * 1_000;
            ...
            body: {
              agents: agents.map((agent) => ({
                ...
                connectionStatus: presenceByAgent.get(agent.agentId)?.connected
                  ...
                  : "offline",
                lastSeenAt:
                  presenceByAgent.get(agent.agentId)?.lastSeenAt ?? null,
              })),
            },
          };
        } catch (error) {
      ...
      if (repository) {
        throw new ApiError(
          503,
          "bounded_agent_directory_unavailable",
          "The durable agent store cannot provide a bounded account directory.",
        );
      }
      const onlineAfter = addSeconds(
        database.clock.now(),
    ...
    if (method === "PUT" && ownerProfileMatch) {
      ...
      enforceHumanControlAttemptRate(principal, agentId);
      const ownedAgent = repository?.findAgentById
        ? await hydrateDurableAgent(agentId)
        : (db.prepare("SELECT * FROM agents WHERE id = ?").get(agentId) as
            | AgentRow
            | undefined);
      if (!ownedAgent || ownedAgent.owner_account_id !== principal.accountId)
        throw new ApiError(404, "agent_not_found", "Owned agent not found.");
      const profileInput = asObject(await readJson(request));
      // Build and validate the exact canonical candidate without advancing the
      // local revision. Semantic replays of a non-idempotent owner PUT return
      // the stored representation and allocate no Firestore/event revision.
      ...
      const profileUnchanged =
        candidate.name === ownedAgent.name &&
        candidate.handle === ownedAgent.handle &&
        candidate.tagline === ownedAgent.tagline &&
        candidate.interests_json === ownedAgent.interests_json &&
        candidate.personality === ownedAgent.personality &&
        candidate.attention_json === ownedAgent.attention_json &&
        candidate.definition_digest === ownedAgent.definition_digest;
      if (profileUnchanged) {
        return { body: { agent: agentFromRow(ownedAgent) } };
      }
      if (repository?.upsertAgent) {
        const result = await durableWrite("agent profile update", async () =>
          repository.upsertAgent!({
            ...
            expectedUpdatedAt: ownedAgent.updated_at,
          }),
        );
        if (!result) {
          throw new ApiError(
            503,
            "agent_store_unavailable",
            "The agent store did not return the committed profile revision.",
          );
        }
        ...
        if (!result.changed) {
          return { body: { agent: agentFromRow(committedCandidate) } };
        }
        ...
        // leave this replica ahead of the authoritative profile. Use the
        // revision selected inside the durable transaction, not the request's
        // pre-transaction clock value.
        database.transaction(() => {
          db.prepare(
            ...
            committedCandidate.name,
            committedCandidate.handle,
            committedCandidate.tagline,
            committedCandidate.interests_json,
            committedCandidate.personality,
            committedCandidate.attention_json,
            committedCandidate.definition_digest,
            committedCandidate.updated_at,
    ...
    const profileProposalListMatch = matchingPath(
      path,
      ...
      if (repository?.listProfileReviewProposals) {
        try {
          ...
          throw new ApiError(
            ...
            error instanceof Error
              ? error.message
              : "The profile review store is unavailable.",
          );
        }
      }
      const owned = db
        .prepare(
          "SELECT 1 AS owned FROM agents WHERE id = ? AND owner_account_id = ?",
        )
    ...
    if (method === "POST" && profileProposalResolveMatch) {
      ...
      if (repository?.resolveProfileReviewProposal) {
        let resolved:
          | Awaited<
              ReturnType<
                NonNullable<MeshrRepository["resolveProfileReviewProposal"]>
              >
            >
          | undefined;
        try {
          ...
          if (
            error instanceof Error &&
            error.message === "profile_proposal_not_found"
          ) {
            throw new ApiError(
              404,
              "profile_proposal_not_found",
              "Profile review proposal not found.",
            );
          }
          if (
            error instanceof Error &&
            error.message === "profile_proposal_not_pending"
          ) {
            throw new ApiError(
              409,
              "profile_proposal_not_pending",
              "This profile review proposal is already resolved.",
            );
          }
          if (
            error instanceof Error &&
          ...
          if (
            error instanceof Error &&
            error.message === "profile_proposal_invalid"
          ) {
            throw new ApiError(
              409,
              "profile_proposal_invalid",
              "The proposed profile is no longer valid.",
            );
          }
          if (
            error instanceof Error &&
      ...
      const resolved = database.transaction(() => {
        ...
        if (
          ...
          String(row.owner_account_id ?? "") !== principal.accountId
        ) {
          throw new ApiError(
            404,
            "profile_proposal_not_found",
        ...
        const requested = JSON.parse(String(row.requested_json)) as Record<
          string,
          unknown
        >;
        ...
        if (decision === "approved") {
          const nextName =
            requested.name === undefined ? next.name : String(requested.name);
          const nextHandle =
          ...
          if (
            ...
            for (const field of ["browse", "rootPosts", "replies"] as const) {
              const value = (requested.attention as Record<string, unknown>)[
                field
              ];
          ...
          db.prepare(
            ...
            String(row.source_digest) || next.definitionDigest,
            resolvedAt,
            agentId,
          );
          next = {
    ...
    if (method === "DELETE" && bindingMatch) {
      ...
      const durableRevocation = repository?.revokeAgent
        ? await durableWrite("agent binding revoke", async () =>
            repository.revokeAgent!(
              agentId,
              now,
              revokeEvent,
              revokeAudit,
        ...
        : undefined;
      if (repository?.revokeAgent && !durableRevocation) {
        throw new ApiError(
          503,
          "agent_store_unavailable",
          "The agent store did not return the revocation result.",
        );
      }
      const revoked =
        durableRevocation && !durableRevocation.changed
          ? { pairings: 0, sessions: 0, pageGrants: 0 }
          : database.transaction(() => {
              const pairings = db
                .prepare(
              ...
              const sessions = db
                .prepare("DELETE FROM agent_sessions WHERE agent_id = ?")
                .run(agentId);
              const pageGrants = db
                .prepare(
                ...
                .run(now, agentId);
              if (
                durableRevocation?.changed ||
                pairings.changes > 0 ||
                sessions.changes > 0 ||
                pageGrants.changes > 0
              ) {
                emitEvent(
                  "agent.disconnected",
                  agentId,
              ...
              return {
                pairings:
                  durableRevocation?.pairings ?? Number(pairings.changes),
                sessions:
                  durableRevocation?.sessions ?? Number(sessions.changes),
                pageGrants:
                  durableRevocation?.pageGrants ?? Number(pageGrants.changes),
              };
            });
      return {
        body: {
    ...
    if (method === "POST" && path === "/v1/webmcp/session") {
      const human = await requireHuman(request, false);
      ...
      const agent = repository?.findAgentById
        ...
        : (db
            .prepare(
              "SELECT * FROM agents WHERE id = ? AND owner_account_id = ?",
            )
      ...
      if (repository?.findActiveWebMcpGrant && webMcpTransfersSession) {
        try {
          ...
          if (durableGrant) {
            ...
            const recovered = await reconcileDurableWebMcpGrant(
              durableGrant,
              human,
            );
            if (recovered) {
      ...
      // satisfy.
      const localRecovery =
        !repository && webMcpTransfersSession
          ? (db
              .prepare(
              ...
              .get(human.sessionHash, agent.id, database.now()) as
              WebMcpGrantRow | undefined)
          : undefined;
      if (localRecovery) {
        const token = webMcpTokenForActiveGrant(
      ...
      // bounded and blocked below only after the recovery path has been
      // exhausted.
      ...
      const transferMeshIds = new Set<string>(
        (
          db
            ...
            .all(agent.id) as Array<{ mesh_id: string }>
        ).map((row) => row.mesh_id),
      );
      if (repository?.listJoinedMeshIdsForAgent) {
        try {
          transferMeshIds.clear();
          for (const meshId of await repository.listJoinedMeshIdsForAgent(
            agent.id,
          )) {
    ...
    if (method === "GET" && path === "/v1/activity/public") {
      const includeAuthorized =
        url.searchParams.get("includeAuthorized") === "1";
      const requestedMeshId = url.searchParams.get("meshId") ?? undefined;
      if (
        requestedMeshId !== undefined &&
        !/^[A-Za-z0-9._:-]{1,128}$/.test(requestedMeshId)
      ) {
        throw new ApiError(
          400,
          "invalid_mesh",
          "The requested mesh is invalid.",
        );
      }
      // Both public and account-authorized activity use the bounded,
      // aggregate-only projection. Private or unlisted activity is refreshed
      // authoritatively because a role change must not leave a stale private
      // mesh in a browser response.
      ...
      if (repository?.loadProjection) {
        try {
          if (includeAuthorized) {
            ...
            await refreshHumanActivityProjection(
              principal.accountId,
              requestedMeshId,
            );
          } else {
          ...
          throw new ApiError(
            ...
            error instanceof Error
              ? error.message
              : "The public projection is unavailable.",
          );
        }
      }
      const projection = repository?.loadProjection
        ? cachedProjection(
            includeAuthorized ? { accountId: principal.accountId } : {},
          )
        : undefined;
      const authorizedMeshIds = projection
        ...
        : includeAuthorized
          ? new Set<string>([
              ...
              ...(
                db
                  .prepare(
                    "SELECT mesh_id AS id FROM mesh_human_roles WHERE account_id = ?",
                  )
                  .all(principal.accountId) as Array<{ id: string }>
              ).map((mesh) => mesh.id),
            ])
          : undefined;
      ...
      const durablePresence = projection
        ? new Map(
            projection.runtimeSessions.map((session) => [
              session.agentId,
              {
                lastSeenAt: session.lastSeenAt,
                connected:
    ...
    if (method === "GET" && path === "/v1/activity/preferences") {
      ...
      if (repository?.listHumanActivityPreferences) {
        try {
          preferences = await repository.listHumanActivityPreferences(
            principal.accountId,
          );
        } catch (error) {
          throw new ApiError(
            ...
            error instanceof Error
              ? error.message
              : "The activity preference store is unavailable.",
          );
        }
      } else {
        const rows = db
    ...
    if (method === "PUT" && activityPreferenceMatch) {
      // This route performs narrow authoritative resource/session checks in
      // the preference transaction below. Avoid hydrating the account-wide
      // projection before the write limiter has a chance to reject a hot
      // caller.
      const principal = await requireHuman(request);
      requireCsrf(request, principal);
      await enforceGovernanceRate(principal.accountId, "activity-preference");
      const kind =
        activityPreferenceMatch[1] as RepositoryHumanActivityPreference["kind"];
      const resourceId = decodeURIComponent(activityPreferenceMatch[2]);
      if (resourceId.length < 1 || resourceId.length > 256) {
        throw new ApiError(
          400,
          "invalid_preference",
          "The activity resource is invalid.",
        );
      }
      const input = asObject(await readJson(request));
      ...
      if (kind === "topic") {
        if (repository?.findTopicById) {
          ...
          const topic = db
            .prepare("SELECT mesh_id FROM topics WHERE id = ?")
            .get(resourceId) as { mesh_id: string } | undefined;
          if (!topic)
            throw new ApiError(
              404,
              "topic_not_found",
      ...
      if (repository) {
        ...
        const patch: RepositoryHumanActivityPreferencePatch = {
          ...
          resourceId,
          meshId,
          ...(requestedWatching === undefined
            ? {}
            : { watching: requestedWatching as boolean }),
          ...(requestedMuted === undefined
            ? {}
            : { muted: requestedMuted as boolean }),
        ...
        try {
          ...
          if (message === "mesh_access_denied") {
            throw new ApiError(
              403,
              "mesh_access_denied",
              "You do not have access to this mesh.",
            );
          }
          ...
          if (message === "activity_preference_limit_reached") {
            throw new ApiError(
              429,
              "activity_preference_limit_reached",
              `An account can store at most ${MAX_ACTIVITY_PREFERENCES_PER_ACCOUNT} activity preferences.`,
            );
          }
        ...
        if (mesh.visibility !== "public") {
          const role = await meshRoleForAuthoritatively(
            principal.accountId,
            meshId,
          );
          if (!role)
        ...
        const current = db
          ...
          { watching: number; muted: number } | undefined;
        preference = {
          accountId: principal.accountId,
          kind,
          resourceId,
          watching:
            requestedWatching === undefined
              ? current?.watching === 1
              : (requestedWatching as boolean),
          muted:
            requestedMuted === undefined
              ? current?.muted === 1
              : (requestedMuted as boolean),
          updatedAt: database.now(),
        };
        if ((preference.watching || preference.muted) && !current) {
    ...
    if (method === "GET" && path === "/v1/live/authorize") {
      ...
      if (authorization.startsWith("Bearer ")) {
        ...
        const meshAccess = await ensureAttentionMeshAccessAuthoritatively(
          agent,
          principal.agentId,
          meshId,
        );
          ...
          body: {
            ...
            cursor: Number(
              (
                db
                  .prepare(
                    "SELECT COALESCE(MAX(sequence), 0) AS sequence FROM events WHERE mesh_id = ?",
                  )
                  .get(meshId) as { sequence: number }
              ).sequence ?? 0,
            ),
          },
        };
    ...
    if (method === "GET" && path === "/v1/meshes") {
      ...
      if (repository?.listMeshDirectoryForAccount) {
        try {
          const directory = await repository.listMeshDirectoryForAccount(
            principal.accountId,
          );
          return {
            body: {
              meshes: directory.map(meshSummaryFromDirectory),
              ...(directory.some((entry) => entry.truncated)
                ? { truncated: true }
                : {}),
            },
          };
          ...
          throw new ApiError(
            ...
            error instanceof Error
              ? error.message
              : "The mesh directory is unavailable.",
          );
        }
      }
      if (repository) {
        throw new ApiError(
          503,
          "bounded_mesh_directory_unavailable",
          "The durable mesh store cannot provide a bounded account directory.",
        );
      }
      const meshRows = db
        .prepare(
        ...
        .all(
          principal.accountId,
          MAX_MESH_DIRECTORY_ENTRIES + 1,
        ) as Array<{ id: string }>;
      const meshes = meshRows
        .slice(0, MAX_MESH_DIRECTORY_ENTRIES)
        .map((raw) =>
          meshSummary(
            readMesh(String(raw.id)),
            principal.accountId,
          ),
        );
      return {
        body: {
          meshes,
          ...(meshRows.length > MAX_MESH_DIRECTORY_ENTRIES
            ? { truncated: true }
            : {}),
        },
      };
    ...
    if (method === "POST" && path === "/v1/meshes") {
      // Agent ownership and the owned-mesh quota are read authoritatively
      // below. A full projection here would let rejected create attempts
      // force the expensive fan-out before reaching the rate guard.
      const principal = await requireHuman(request);
      requireCsrf(request, principal);
      ...
      const defaultJoinPolicy =
        visibility === "public" ? "open" : "invite_only";
      const joinPolicy =
        input.joinPolicy === undefined ? defaultJoinPolicy : input.joinPolicy;
      if (
        joinPolicy !== "open" &&
        joinPolicy !== "approval" &&
      ...
      await enforceGovernanceRate(principal.accountId, "mesh-create");
      ...
      if (repository?.listAgentsForAccount) {
        try {
          durableOwnedAgents = await repository.listAgentsForAccount(
            principal.accountId,
          );
          if (repository.listMeshDirectoryForAccount) {
            durableOwnedMeshes = await repository.listMeshDirectoryForAccount(
              principal.accountId,
            );
          }
        } catch (error) {
        ...
        input.agentIds === undefined
          ...
          : (() => {
              if (
                ...
                input.agentIds.length > MAX_AGENTS_PER_ACCOUNT
              ) {
                throw new ApiError(
                  400,
                  "invalid_mesh",
                  ...
                  input.agentIds.map((value) => {
                    if (
                      ...
                      throw new ApiError(
                        400,
                        "invalid_mesh",
                        "agentIds must contain valid agent IDs.",
                      );
                    }
                    return value.trim();
                  }),
      ...
      const now = database.now();
      const requestHash = sha256(
        JSON.stringify({ name, description, visibility, joinPolicy, agentIds }),
      );
      const meshSeed = sha256(
        `mesh:create:${principal.accountId}:${idempotencyKey}`,
      );
      ...
      const existingMeshForIdempotency:
      ...
      if (!repository) {
        ...
        const topicExists = db
          .prepare("SELECT 1 FROM topics WHERE id = ? AND mesh_id = ?")
          .get(topicId, meshId);
        ...
        if (existing) {
          const matches =
            topicExists &&
            existing.owner_account_id === principal.accountId &&
            existing.name === name &&
            existing.description === description &&
            existing.visibility === visibility &&
            existing.join_policy === joinPolicy &&
          ...
          if (!matches)
            throw new ApiError(
              409,
              "idempotency_conflict",
              "This idempotency key was already used for a different mesh.",
            );
      ...
      let summary: ReturnType<typeof meshSummary> | ReturnType<typeof meshSummaryFromDirectory>;
      if (repository?.findMeshDirectoryEntryForAccount) {
        let entry: RepositoryMeshDirectoryEntry | null;
        try {
          entry = await repository.findMeshDirectoryEntryForAccount(
            mesh.id,
            principal.accountId,
          );
        } catch (error) {
        ...
        if (!entry)
          throw new ApiError(404, "mesh_not_found", "Mesh not found.");
        summary = meshSummaryFromDirectory(entry);
      } else {
        summary = meshSummary(readMesh(mesh.id), principal.accountId);
      }
      return {
        status: durableMeshDuplicate ? 200 : 201,
        body: {
          mesh: summary,
          topic: summary.topics.find((topic) => topic.id === mesh.topicId),
        },
      };
    }

    const meshGovernanceMatch = matchingPath(
      path,
      /^\/v1\/meshes\/([^/]+)\/governance$/,
    );
    if (meshGovernanceMatch && (method === "GET" || method === "PUT")) {
      // Both reads and writes use narrow authoritative mesh/role/directory
      // operations below. Authentication must never hydrate the account-wide
      // projection before CSRF and rate checks.
      const principal =
        method === "GET"
          ? await requireHuman(request, false, { touchSession: false })
          : await requireHuman(request, false);
      if (method === "GET")
        enforceExpensiveAccountReadRate(principal.accountId);
      else requireCsrf(request, principal);
      const meshId = decodeURIComponent(meshGovernanceMatch[1]);
      if (method === "PUT") enforceHumanControlAttemptRate(principal);
      ...
      if (method === "GET") {
        ...
        if (!role && mesh.visibility !== "public") {
          throw new ApiError(
            403,
            "mesh_access_denied",
            "You cannot view this mesh governance.",
          );
        }
        const authoritativeDirectoryAvailable = Boolean(
          repository?.findMeshDirectoryEntryForAccount ||
            repository?.listMeshDirectoryForAccount,
        );
        const localRoleRows = authoritativeDirectoryAvailable
          ? []
          : (db
              .prepare(
                `SELECT r.account_id, r.role, r.created_at, r.updated_at,
              ...
              .all(meshId, MAX_MESH_DETAIL_ROLE_ROWS + 1) as Array<
              Record<string, string>
            >);
        const localRolesTruncated =
          localRoleRows.length > MAX_MESH_DETAIL_ROLE_ROWS;
        const roles = localRoleRows
          .slice(0, MAX_MESH_DETAIL_ROLE_ROWS)
          .map((raw) => {
            const row = raw as Record<string, string>;
            const roleSummary = {
              accountId: row.account_id,
        ...
        if (
          ...
          let entry: RepositoryMeshDirectoryEntry | null;
          try {
            entry = repository.findMeshDirectoryEntryForAccount
              ? await repository.findMeshDirectoryEntryForAccount(
                  meshId,
                  principal.accountId,
                )
              :
                (await repository.listMeshDirectoryForAccount!(
                  principal.accountId,
                )).find((candidate) => candidate.mesh.meshId === meshId) ?? null;
          } catch (error) {
            throw new ApiError(
              503,
              "governance_store_unavailable",
        ...
        return {
          body: {
            mesh: meshSummary(finalMesh, principal.accountId),
            role: finalRole,
            roles,
            ...(localRolesTruncated ? { truncated: true } : {}),
          },
        };
      }
      ...
      for (const key of Object.keys(input)) {
        if (
          !["name", "description", "visibility", "joinPolicy"].includes(key)
        ) {
          throw new ApiError(400, "invalid_request", `${key} is not allowed.`);
        }
      }
      const name =
        input.name === undefined
          ? undefined
          : requiredString(input, "name", { max: 80 });
      const description =
        input.description === undefined
          ? undefined
          : (optionalString(input, "description", 500) ?? "");
      const visibility =
        input.visibility === undefined ? undefined : input.visibility;
      const joinPolicy =
        input.joinPolicy === undefined ? undefined : input.joinPolicy;
      if (
      ...
      if (
        ...
        !["open", "approval", "invite_only"].includes(String(joinPolicy))
      ) {
        throw new ApiError(400, "invalid_mesh", "joinPolicy is invalid.");
      }
      if (
        ...
        throw new ApiError(
          400,
          "invalid_mesh",
          "Set at least one governance field.",
        );
      }
      const governanceUnchanged =
        (name ?? mesh.name) === mesh.name &&
        (description ?? mesh.description) === mesh.description &&
        (visibility ?? mesh.visibility) === mesh.visibility &&
        (joinPolicy ?? mesh.join_policy) === mesh.join_policy;
      if (governanceUnchanged) {
        if (repository?.findMeshDirectoryEntryForAccount) {
          let entry: RepositoryMeshDirectoryEntry | null;
          try {
            entry = await repository.findMeshDirectoryEntryForAccount(
          ...
          if (!entry)
            throw new ApiError(404, "mesh_not_found", "Mesh not found.");
          return { body: { mesh: meshSummaryFromDirectory(entry) } };
        }
        return {
          body: { mesh: meshSummary(mesh, principal.accountId) },
        };
      }
      const now = database.now();
      ...
      const governancePatch: RepositoryMeshGovernancePatch = {
        ...
        ...(visibility === undefined
          ...
          : {
              visibility:
                visibility as RepositoryMeshGovernancePatch["visibility"],
            }),
        ...(joinPolicy === undefined
          ? {}
          : {
              admission:
                joinPolicy as RepositoryMeshGovernancePatch["admission"],
            }),
        updatedAt: now,
        actingAccountId: principal.accountId,
        humanSessionHash: principal.sessionHash,
      ...
      if (repository?.updateMeshGovernance) {
        try {
          committedMesh =
            await repository.updateMeshGovernance(governancePatch);
        } catch (error) {
          const message =
            error instanceof Error
              ? error.message
        ...
        committedMesh = {
          ...
          visibility: (visibility ??
            mesh.visibility) as RepositoryMeshInput["visibility"],
          admission: (joinPolicy ??
            mesh.join_policy) as RepositoryMeshInput["admission"],
          lifecycle: "active",
          createdAt: mesh.created_at,
          updatedAt: now,
          actingAccountId: principal.accountId,
      ...
      database.transaction(() => {
        ...
        emitAudit({
          ...
          data: governancePayload,
          durable: Boolean(
            repository?.updateMeshGovernance ?? repository?.upsertMesh,
          ),
        });
        emitEvent(
          "mesh.governance.updated",
          null,
          meshId,
          null,
          governancePayload,
      ...
      return {
        body: { mesh: meshSummary(readMesh(meshId), principal.accountId) },
      };
    }

    const meshTopicListMatch = matchingPath(
      path,
      /^\/v1\/meshes\/([^/]+)\/topics$/,
    );
    if (meshTopicListMatch && (method === "GET" || method === "POST")) {
      // The durable GET path returns the authoritative directory entry and
      // neither method needs the account-wide projection.
      const principal =
        method === "GET"
          ? await requireHuman(request, false, { touchSession: false })
          : await requireHuman(request, false);
      ...
      if (method === "GET") {
        ...
        if (!role && mesh.visibility !== "public") {
          throw new ApiError(
            403,
            "mesh_access_denied",
            "You cannot view topics in this mesh.",
          );
        }
        return {
          body: { topics: meshSummary(mesh, principal.accountId).topics },
        };
      }
          ...
          : (() => {
              if (!Array.isArray(input.tags) || input.tags.length > 12) {
                throw new ApiError(
                  400,
                  "invalid_topic",
                  "tags must be an array with at most 12 entries.",
                );
              }
              return input.tags.map((tag) => {
                if (
                  ...
                  throw new ApiError(
                    400,
                    "invalid_topic",
                    "Each topic tag must be between 1 and 32 characters.",
                  );
                }
      ...
      if (!repository) {
        database.transaction(() => {
          ...
          if (
            db
              .prepare("SELECT 1 FROM topics WHERE mesh_id = ? AND name = ?")
              .get(meshId, name)
          ) {
            throw new ApiError(
              409,
    ...
    const meshTopicItemMatch = matchingPath(
      path,
      /^\/v1\/meshes\/([^/]+)\/topics\/([^/]+)$/,
    );
    if (meshTopicItemMatch && (method === "PUT" || method === "DELETE")) {
      ...
      if (method === "DELETE") {
        ...
        if (!repository) {
          database.transaction(() => {
            ...
            if (
              db
                .prepare("SELECT 1 FROM posts WHERE topic_id = ? LIMIT 1")
                .get(topicId)
            ) {
              throw new ApiError(
                409,
            ...
            db.prepare("DELETE FROM topics WHERE id = ? AND mesh_id = ?").run(
              topicId,
              meshId,
            );
            emitAudit({
      ...
      const title =
        input.title === undefined
          ? currentTopic.title
          : requiredString(input, "title", { max: 100 });
      const description =
        input.description === undefined
      ...
      if (!repository) {
        database.transaction(() => {
          if (
            db
              .prepare(
                "SELECT 1 FROM topics WHERE mesh_id = ? AND name = ? AND id <> ?",
              )
              .get(meshId, name, topicId)
          ) {
            throw new ApiError(
              409,
              "topic_name_taken",
    ...
    const meshInvitationListMatch = matchingPath(
      path,
      /^\/v1\/meshes\/([^/]+)\/invitations$/,
    );
    if (meshInvitationListMatch && (method === "GET" || method === "POST")) {
      ...
      if (method === "GET") {
        ...
        // the response.
        const terminalPrincipal = await requireHuman(request, false, {
          touchSession: false,
        });
        await requireMeshRole(terminalPrincipal.accountId, meshId, [
          "owner",
          "steward",
        ]);
        return {
          body: { invitations: invitations.map(meshInvitationRepresentation) },
        };
      }
      await enforceGovernanceRate(principal.accountId, "mesh-invitation");
      const input = asObject(await readJson(request));
      for (const key of Object.keys(input)) {
      ...
      if (invitedAgentId && repository?.findAgentById) {
        ...
        invitedAgentId &&
        !db.prepare("SELECT 1 FROM agents WHERE id = ?").get(invitedAgentId)
      ) {
        throw new ApiError(404, "agent_not_found", "Agent not found.");
      }
      const rawExpiry = input.expiresInSeconds;
      const expiresInSeconds =
        rawExpiry === undefined ? 7 * 24 * 60 * 60 : rawExpiry;
      if (
        ...
        throw new ApiError(
          400,
          "invalid_request",
          "expiresInSeconds must be between 60 seconds and 30 days.",
        );
      }
      const nowDate = database.clock.now();
      const now = nowDate.toISOString();
      ...
      if (repository?.createMeshInvitation) {
        try {
          ...
          if (
            error instanceof Error &&
            error.message === "invitation_limit_reached"
          ) {
            throw new ApiError(
              429,
              "invitation_limit_reached",
              "This mesh has reached the 50 active-invitation limit.",
            );
          }
          throw new ApiError(
            503,
        ...
        if (!repository.listMeshInvitations) {
          const localMesh = db
            .prepare("SELECT 1 FROM meshes WHERE id = ?")
            .get(meshId);
          if (localMesh) {
            database.transaction(() => {
              db.prepare(
    ...
    if (method === "POST" && meshInvitationRevokeMatch) {
      ...
      if (repository?.revokeMeshInvitation) {
        try {
          ...
          if (
            error instanceof Error &&
            error.message === "invitation_not_active"
          ) {
            throw new ApiError(
              409,
              "invitation_not_active",
        ...
        const result = database.transaction(() => {
          ...
          if (outcome.changes !== 1)
            throw new ApiError(
              404,
              "invitation_not_found",
              "Invitation not found or already inactive.",
            );
          emitAudit({
            auditId,
            createdAt: now,
    ...
    if (
      ...
      meshRoleInvitationCollectionMatch
    ) {
      const principal =
        method === "GET"
          ? await requireHuman(request, false, { touchSession: false })
      ...
      if (method === "GET") {
        ...
        try {
          invitations =
            await roleInvitationStore.listMeshRoleInvitations!(meshId);
        } catch (error) {
        ...
        return {
          body: {
            invitations: invitations.map(meshRoleInvitationRepresentation),
          },
        };
      }
      await enforceGovernanceRate(principal.accountId);
      ...
      if (role !== "owner" && role !== "steward" && role !== "observer") {
        throw new ApiError(
          400,
          "invalid_role",
          "role must be owner, steward, or observer.",
        );
      }
      const rawExpiry = input.expiresInSeconds;
      const expiresInSeconds =
    ...
    if (method === "GET" && path === "/v1/account/role-invitations") {
      ...
      enforceExpensiveAccountReadRate(principal.accountId);
      await enforceGovernanceRate(principal.accountId, "role-invitation-read");
      const emailHashes = roleInvitationEmailHashes(
        normalizeEmail(principal.email),
      );
      const pages = await Promise.all(
        emailHashes.map((emailHash) =>
          roleInvitationStore.listMeshRoleInvitationsForEmail!(emailHash),
        ),
      );
      const unique = new Map<string, RepositoryMeshRoleInvitation>();
      for (const invitation of pages.flat())
        unique.set(invitation.invitationId, invitation);
    ...
    if (method === "POST" && roleInvitationAcceptMatch) {
      ...
      try {
        for (const emailHash of roleInvitationEmailHashes(
          normalizeEmail(principal.email),
        )) {
      ...
      let accepted!: Awaited<
        ReturnType<NonNullable<MeshrRepository["acceptMeshRoleInvitation"]>>
      >;
      try {
    ...
    const meshRoleCollectionMatch = matchingPath(
      path,
      /^\/v1\/meshes\/([^/]+)\/roles$/,
    );
    ...
    const meshRoleMatch = matchingPath(
      path,
      /^\/v1\/meshes\/([^/]+)\/roles\/([^/]+)$/,
    );
    if ((method === "PUT" || method === "DELETE") && meshRoleMatch) {
      ...
      await enforceGovernanceRate(principal.accountId, "mesh-role-mutation");
      let targetAccountExists = Boolean(
      ...
      if (!targetAccountExists && repository?.findAccountById) {
        try {
          targetAccountExists = Boolean(
            await repository.findAccountById(accountId),
          );
        } catch (error) {
          throw new ApiError(
            503,
      ...
      if (method === "DELETE") {
        ...
        if (current === "owner" && !repository) {
          const ownerCount = db
            .prepare(
              "SELECT COUNT(*) AS count FROM mesh_human_roles WHERE mesh_id = ? AND role = 'owner'",
            )
            .get(meshId) as { count: number };
        ...
        if (repository) {
          db.prepare(
            "DELETE FROM mesh_human_roles WHERE mesh_id = ? AND account_id = ?",
    ...
    if (method === "DELETE" && meshAgentMembershipMatch) {
      ...
      const membershipMutation = repository?.upsertMeshAgentMembership
        ? await durableWrite("mesh membership removal", async () =>
            repository.upsertMeshAgentMembership!({
              meshId,
              agentId,
              status: "removed",
              attentionPolicy: {},
        ...
        : undefined;
      if (repository?.upsertMeshAgentMembership && !membershipMutation) {
        throw new ApiError(
          503,
          "governance_store_unavailable",
          "The governance store did not return the membership result.",
        );
      }
      if (repository) {
        if (membershipMutation?.changed) {
          // The durable removal CAS preserves the membership's original
          // attention policy, admission provenance, and join timestamp. Mirror
          // only the fields committed by that command; if this replica never
          // observed the row, do not invent historical provenance for it.
          database.transaction(() => {
            db.prepare(
              "DELETE FROM mesh_members WHERE mesh_id = ? AND agent_id = ?",
            ).run(meshId, agentId);
    ...
    const joinRequestListMatch = matchingPath(
      path,
      /^\/v1\/meshes\/([^/]+)\/join-requests$/,
    );
    if (method === "GET" && joinRequestListMatch) {
      ...
      const requests = await Promise.all(
        durableRequests.map(async (joinRequest) => {
          const localAgent = db
            .prepare("SELECT id, name, handle FROM agents WHERE id = ?")
            .get(joinRequest.agentId) as
            { id: string; name: string; handle: string } | undefined;
          ...
          if (!agent && repository?.findAgentById) {
            ...
            if (durableAgent) {
              agent = {
                id: durableAgent.agentId,
                name: durableAgent.name,
                handle: durableAgent.handle,
              };
            }
          }
          return {
    ...
    if (method === "POST" && joinRequestResolveMatch) {
      ...
      if (
        !pendingJoinRequest ||
        pendingJoinRequest.meshId !== meshId ||
      ...
      const outcome = database.transaction(() => {
        // The request body may arrive slowly. Serialize the final session and
        // role checks with the resolution write so logout or demotion cannot
        // race an approval that was admitted earlier.
        assertCurrentHumanSession(principal);
        const currentRole = meshRoleFor(principal.accountId, meshId);
        if (currentRole !== "owner" && currentRole !== "steward") {
          throw new ApiError(
            403,
            "mesh_governance_denied",
            "You do not have the required mesh role.",
        ...
        if (decision === "approved") {
          const currentMesh = db.prepare(
            "SELECT lifecycle, join_policy FROM meshes WHERE id = ?",
          ).get(meshId) as
            | {
            ...
            | undefined;
          if (!currentMesh || currentMesh.lifecycle !== "active") {
            throw new ApiError(
              409,
              "mesh_unavailable",
              "This mesh is not accepting new members.",
          ...
          if (currentMesh.join_policy !== "approval") {
            throw new ApiError(
              409,
              "mesh_admission_changed",
              "This mesh no longer accepts approval-based join requests.",
          ...
          requireBrowsePolicy(currentAgentForCommit(pending.agent_id));
        }
        db.prepare(
      ...
      await durableWrite("mesh join request resolution", async () => {
        ...
        if (resolvedRequest.status === "approved") {
          const joinedAgent = db
            .prepare("SELECT attention_json FROM agents WHERE id = ?")
            .get(resolvedRequest.agent_id) as
            { attention_json: string } | undefined;
          await repository?.upsertMeshAgentMembership?.({
            meshId: resolvedRequest.mesh_id,
            agentId: resolvedRequest.agent_id,
            status: "joined",
            attentionPolicy: joinedAgent
              ? (JSON.parse(joinedAgent.attention_json) as Record<
                  string,
                  unknown
                >)
              : {},
    ...
    // taken.
    const postReportMatch = matchingPath(
      path,
      /^\/v1\/posts\/([^/]+)\/report$/,
    );
    if (method === "POST" && postReportMatch) {
      const principal = await requireHuman(request);
      requireCsrf(request, principal);
      await enforceGovernanceRate(principal.accountId, "moderation-report");
      ...
      await requireMeshRole(principal.accountId, post.meshId, [
        "owner",
        "steward",
      ]);
      const idempotencyKey = requireIdempotencyKey(request);
      ...
      const now = database.now();
      const requestHash = sha256(
        JSON.stringify({ postId, meshId: post.meshId, reason }),
      );
      const reportDigest = sha256(
        `moderation-report:${principal.accountId}:${postId}:${idempotencyKey}`,
      );
      ...
      if (!mutation?.moderationCase || !mutation.post) {
        throw new ApiError(
          503,
          "moderation_store_unavailable",
          "The moderation store did not return the committed case.",
        );
      }
      if (repository) {
    ...
    const moderationListMatch = matchingPath(
      path,
    ...
    if (method === "GET" && moderationListMatch) {
      ...
      if (!mesh) throw new ApiError(404, "mesh_not_found", "Mesh not found.");
      const role = await requireMeshRole(principal.accountId, meshId, [
        "owner",
        "steward",
      ]);
      ...
      if (
        stateParam &&
        !["queued", "reviewing", "resolved", "appealed"].includes(stateParam)
      ) {
        throw new ApiError(
          400,
          "invalid_moderation_state",
          "state must be queued, reviewing, resolved, or appealed.",
        );
      }
      const cursor = parseCursor(url.searchParams.get("after"));
      const limit = parsePositiveInteger(
      ...
      const page = await listModerationCasesForRoute({
        ...
        after: cursor
          ? { updatedAt: cursor.createdAt, caseId: cursor.id }
          : undefined,
        limit,
      });
      const entries = await Promise.all(
        page.cases.map(async (moderationCase) => {
          const post = await findPostForModeration(moderationCase.postId);
          return moderationCaseRepresentation(
            moderationCase,
            post,
            role === "owner" || role === "steward",
          );
        }),
      );
      ...
      return {
        body: {
          ...
          nextCursor: page.nextAfter
            ? encodeCursor({
                created_at: page.nextAfter.updatedAt,
                id: page.nextAfter.caseId,
              })
            : null,
        },
      };
    ...
    if (method === "POST" && moderationActionMatch) {
      ...
      if (
        ...
        throw new ApiError(
          400,
          "invalid_moderation_action",
          "action must be start_review, publish, quarantine, remove, or redact.",
        );
      }
      ...
      const reasonProvided = Object.prototype.hasOwnProperty.call(
        input,
        "reason",
      );
      const reasonOverride = reasonProvided
        ? optionalString(input, "reason", 200)
        : undefined;
      ...
      const actionDigest = sha256(
        `moderation:${principal.accountId}:${meshId}:${caseId}:${idempotencyKey}`,
      );
      ...
      const parentPost = post.parentPostId
        ? await findPostForModeration(post.parentPostId)
        : null;
      const nextCaseState: RepositoryModerationCase["state"] =
        action === "start_review" ? "reviewing" : "resolved";
      const nextPostState =
        action === "publish"
          ? "published"
          : action === "quarantine"
      ...
      const redactedBody =
        action === "redact"
          ? "[Content redacted by mesh moderation]"
          : undefined;
        ...
        async () =>
          action === "start_review"
            ...
            : await roleInvitationStore.updatePostModeration!({
                caseId,
                postId: post.postId,
                state: nextPostState as
                  "published" | "quarantined" | "removed" | "redacted",
                reason,
                body: redactedBody,
    ...
    if (method === "DELETE" && path === "/v1/session") {
      ...
      await durableWrite("human session revoke", async () => {
        await repository?.revokeHumanSession?.(
          principal.sessionHash,
          database.now(),
        );
    ...
    if (method === "POST" && path === "/v1/pairings") {
      ...
      for (let attempt = 0; attempt < 8; attempt += 1) {
        ...
        const collision = db
          .prepare("SELECT 1 AS found FROM pairings WHERE code = ?")
          .get(code);
        if (!collision) break;
        code = "";
      }
    ...
    if (
      ...
      if (!id || id.length > 128) {
        throw new ApiError(
          400,
          "invalid_pairing_id",
          "Pairing id is invalid.",
        );
      }
      enforcePairingStatusRate(request, id);
      ...
      if (
        !activeSession &&
        pairing.agent_id &&
        repository?.findActiveRuntimeSessionForAgent
      ) {
        try {
          const durableSession =
    ...
    if (method === "GET" && path === "/v1/pairings/lookup") {
      enforcePairingLookupSourceRate(request);
      const principal = await requireHuman(request);
      enforcePairingLookupAccountRate(principal.accountId);
      ...
      if (!found)
        throw new ApiError(404, "pairing_not_found", "Pairing not found.");
      const pairing = await expirePendingPairing(found);
      return { body: { pairing: pairingRepresentation(pairing) } };
    }

    const approveMatch = matchingPath(
      path,
      /^\/v1\/pairings\/([^/]+)\/approve$/,
    );
    if (method === "POST" && approveMatch) {
        ...
        if (
          ...
          if (!existingAgent) {
            throw new ApiError(
              503,
              "agent_store_unavailable",
              "The approved agent is not available yet.",
            );
          }
          return {
            body: {
      ...
      if (!storedProfile && !suppliedProfile) {
        throw new ApiError(
          400,
          "profile_required",
          "An agent profile is required to approve pairing.",
        );
      }
      ...
      const durableBrowseActions = profile.attention.browse !== "mentions";
      if (
        (autonomousPosting || durableBrowseActions) &&
        input.acknowledgeAutonomous !== true
      ) {
        throw new ApiError(
          400,
          "autonomous_acknowledgement_required",
          "Confirm that this agent may join or follow social spaces and, when configured, post autonomously before approving it.",
        );
      }
      const now = database.now();
      ...
      if (
        matchingAgentBefore &&
        matchingAgentBefore.owner_account_id !== principal.accountId
      ) {
        throw new ApiError(
          409,
          "handle_unavailable",
      ...
      try {
        database.transaction(() => {
          ...
          if (
            matchingAgent &&
            matchingAgent.owner_account_id !== principal.accountId
          ) {
            throw new ApiError(
              409,
              "handle_unavailable",
          ...
          if (matchingAgent) {
            ...
            db.prepare("DELETE FROM agent_sessions WHERE agent_id = ?").run(
              agentId,
            );
            db.prepare(
              `UPDATE webmcp_grants SET revoked_at = ?
    ...
    const challengeMatch = matchingPath(
      path,
      /^\/v1\/pairings\/([^/]+)\/challenges$/,
    );
      ...
      if (requestedSessionId !== undefined) {
        ...
        if (active) {
          const parsed = Date.parse(
            String((active as { expires_at?: unknown }).expires_at ?? ""),
          );
        ...
        if (!repository && !active && pairing.agent_id) {
          const expired = db
            ...
            { expires_at: string; authority_epoch: number } | undefined;
        ...
        if (repository?.findRuntimeSessionById) {
          try {
            const durableSession =
              await repository.findRuntimeSessionById(requestedSessionId);
            if (
              durableSession &&
              ...
              const successor = await repository.findRuntimeSessionById(
                durableSession.supersedingSessionId,
              );
              if (
                successor &&
            ...
            durableRequestedSessionSuperseded =
              durableSession?.status === "superseded";
            ...
            expiredPredecessorRecovery = Boolean(
              ...
              durableSession?.status === "active" &&
              Date.parse(durableSession?.expiresAt ?? "") <=
                Date.parse(database.now()),
            );
            active =
              durableSessionBelongsToPairing &&
              durableSession?.status === "active" &&
        ...
        if (recoverySuccessorId) {
          ...
          const local = db
            ...
            { status: string; superseded_by: string | null } | undefined;
        ...
        if (recoverySuccessorId) {
          ...
          throw new ApiError(
            401,
            "session_invalid",
            "The requested runtime session is not active.",
          );
        }
        ...
        const allowEarlyValidationRenewal =
          process.env.MESHR_DATABASE_CUTOVER_MODE?.trim().toLowerCase() ===
          "validation";
        if (
          !allowEarlyValidationRenewal &&
          !recoverySuccessorId &&
          !expiredPredecessorRecovery &&
          active &&
    ...
    if (method === "POST" && path === "/v1/agent-sessions") {
      ...
      if (repository) {
        try {
          ...
          if (
            error instanceof Error &&
            error.message === "session_superseded"
          ) {
            throw new ApiError(
              401,
              "session_superseded",
          ...
          if (
            error instanceof Error &&
            error.message === "page_authority_active"
          ) {
            throw new ApiError(
              409,
      ...
      const projectLocalSession = () =>
        database.transaction(() => {
          ...
          if (
            !currentPairing ||
            (currentPairing.status !== "approved" &&
              currentPairing.status !== "claimed") ||
          ...
          const authorityEpoch =
            authoritativeEpoch ??
            advanceAuthority(claimedAgentId, "native", sessionId, now);
          if (authoritativeEpoch !== undefined) {
            db.prepare(
              `INSERT INTO agent_authority(agent_id, epoch, authority_kind, session_id, updated_at)
             VALUES(?, ?, 'native', ?, ?)
      ...
      try {
        ...
        console.warn(
          "runtime session committed durably but local projection refresh failed",
          error,
        );
      }
    ...
    if (method === "POST" && path === "/v1/agent-sessions/heartbeat") {
      ...
      if (refreshed.changes !== 1) {
        throw new ApiError(
          401,
          "agent_authentication_failed",
          "Agent token is no longer active.",
        );
      }
      const session = db
        .prepare(
    ...
    if (method === "POST" && path === "/v1/agent-sessions/renew") {
      ...
      const pairingId = requiredString(input, "pairingId", { max: 128 });
      enforceEndpointRate(
        pairingRenewalPairLimiter,
        `pairing:${pairingId}`,
        "session_rate_limited",
        "Too many renewal attempts for this binding. Retry after the indicated delay.",
      );
      const challengeId = requiredString(input, "challengeId", { max: 128 });
      ...
      if (
        ...
        !challenge.message.startsWith(
          `meshr-agent-session:v1:renew:${pairing.id}:${sessionId}:`,
        )
      ) {
        throw new ApiError(
          401,
          "challenge_invalid",
          "Challenge is missing, expired, or not bound to this session.",
        );
      }
      if (
        !verifyEd25519Signature(
      ...
      if (repository?.findRuntimeSessionById) {
        try {
          const durableCurrent =
            await repository.findRuntimeSessionById(sessionId);
          const recoverableCandidate =
            durableCurrent?.status === "superseded"
              ? renewalCandidates.find(
                  (candidate) =>
                    candidate.sessionId === durableCurrent.supersedingSessionId,
                )
              : undefined;
          ...
          if (
            !recoverable &&
            (!currentBefore ||
              currentBefore.authority_epoch !== durableCurrent.authorityEpoch)
          ) {
      ...
      const recoveryPredecessorStatus =
        currentBeforeRow?.status ?? "superseded";
      ...
      const canRecover =
        durableRecovery ||
        (recoveryPredecessorStatus === "superseded" &&
          Boolean(localRecoverySuccessor));
      if (canRecover) {
        ...
        if (!challenge.usedAt) {
          const consumed = await consumePairingChallenge(
        ...
        if (repository?.findRuntimeSessionById) {
          try {
            const durableSuccessor =
              await repository.findRuntimeSessionById(replacementSessionId);
            if (
              durableSuccessor &&
          ...
          sha256(replacementToken) ===
            (
              db
                ...
                .get(replacementSessionId) as { token_hash: string } | undefined
            )?.token_hash
        ) {
          const recoveredAgent = db
            .prepare("SELECT * FROM agents WHERE id = ?")
            .get(pairing.agent_id) as unknown as AgentRow | undefined;
          if (!recoveredAgent)
            throw new ApiError(
              503,
              "session_store_unavailable",
              "The agent projection is unavailable.",
            );
          return {
            status: 200,
            body: {
        ...
        throw new ApiError(
          401,
          "challenge_invalid",
          "The renewal transaction could not be recovered.",
        );
      }
      if (!currentBeforeRow) {
        throw new ApiError(
      ...
      const replaced = database.transaction(() => {
        ...
        emitEvent(
          ...
          {
            sessionId: replacementSessionId,
            runtimeKind: current.runtime_kind,
            eventId: renewalEventId,
            occurredAt: now,
            durable: Boolean(repository),
          },
      ...
      const agent = db
        .prepare("SELECT * FROM agents WHERE id = ?")
        .get(replaced.agent_id) as unknown as AgentRow;
      return {
        status: 201,
        body: {
          token: replacementToken,
    ...
    if (method === "GET" && path === "/v1/public/meshes") {
      ...
      if (repository?.listPublicMeshes) {
        try {
          ...
          return {
            body: {
              meshes: publicMeshes.meshes.map((mesh) => ({
                id: mesh.meshId,
                name: mesh.name,
                description: mesh.description,
                visibility: mesh.visibility,
              ...
              ...(publicMeshes.truncated ? { truncated: true } : {}),
            },
          };
        } catch (error) {
          throw new ApiError(
            error instanceof Error && error.message === "mesh_not_found"
              ? 404
              : 503,
            error instanceof Error && error.message === "mesh_not_found"
              ? "mesh_not_found"
            ...
            error instanceof Error
              ? error.message
              : "The public mesh directory is unavailable.",
          );
        }
      }
      const meshes = db
        ...
        .all(MAX_MESH_DIRECTORY_ENTRIES + 1)
        .map((row) => {
      ...
      return {
        body: {
          meshes: meshes.slice(0, MAX_MESH_DIRECTORY_ENTRIES),
          ...(meshes.length > MAX_MESH_DIRECTORY_ENTRIES
            ? { truncated: true }
            : {}),
        },
      };
    ...
    const publicTopicsMatch = matchingPath(
      path,
      /^\/v1\/public\/meshes\/([^/]+)\/topics$/,
    );
    if (method === "GET" && publicTopicsMatch) {
      ...
      if (repository?.listPublicTopics) {
        try {
          ...
          return {
            body: {
              topics: publicTopics.topics.map((topic) => ({
                id: topic.topicId,
                meshId: topic.meshId,
                name: topic.name,
                title: topic.title,
              ...
              ...(publicTopics.truncated ? { truncated: true } : {}),
            },
          };
        } catch (error) {
          throw new ApiError(
            ...
            error instanceof Error
              ? error.message
              : "The public topic directory is unavailable.",
          );
        }
      }
      const mesh = db
        .prepare(
          "SELECT id FROM meshes WHERE id = ? AND visibility = 'public' AND lifecycle = 'active'",
        )
        .get(meshId);
      if (!mesh)
        throw new ApiError(404, "mesh_not_found", "Public mesh not found.");
      const topics = db
        .prepare(
          `SELECT id, mesh_id, name, title, description, tags_json, created_at
           FROM topics WHERE mesh_id = ? ORDER BY title, id LIMIT ?`,
        )
        .all(meshId, MAX_TOPICS_PER_MESH + 1)
        .map((raw) => {
          const row = raw as Record<string, string>;
          return {
            id: row.id,
      ...
      return {
        body: {
          topics: topics.slice(0, MAX_TOPICS_PER_MESH),
          ...(topics.length > MAX_TOPICS_PER_MESH
            ? { truncated: true }
            : {}),
        },
      };
    ...
    if (path.startsWith("/v1/webmcp/")) {
      ...
      if (method === "GET" && path === "/v1/webmcp/meshes") {
        ...
        if (repository?.listMeshesForAgent) {
          ...
          try {
            visible = await repository.listMeshesForAgent(principal.agentId, {
              limit: MODEL_MESH_DIRECTORY_LIMITS.meshes + 1,
              browse,
            });
          } catch (error) {
            throw new ApiError(
          ...
          const currentBrowse = requireBrowsePolicy(currentAgent);
          if (currentBrowse !== browse) {
            throw new ApiError(
              409,
              "attention_policy_changed",
              "The agent's browse policy changed while meshes were being read; retry with the current policy.",
            );
          }
          browse = currentBrowse;
          return {
            body: boundedModelMeshDirectory(
              visible
                .filter(({ mesh, joined }) =>
                  browse === "joined"
                    ? joined
        ...
        const meshes = db
          ...
          .all(
            principal.agentId,
            principal.agentId,
            MODEL_MESH_DIRECTORY_LIMITS.meshes + 1,
          )
          .map((raw) => {
        ...
        await revalidatePostReadAuthority(principal);
        const currentAgent = currentAgentForCommit(principal.agentId);
        if (!currentAgent) {
          throw new ApiError(
            401,
            "agent_authentication_failed",
      ...
      if (method === "GET" && path === "/v1/webmcp/activity") {
        ...
        let meshSelectionTruncated = false;
        ...
        if (rawMeshId !== null) {
          ...
          authorizedMeshIds = new Set([meshId]);
        } else if (repository?.listMeshesForAgent) {
          try {
            const visible = (
              await repository.listMeshesForAgent(principal.agentId, {
                limit: WEBMCP_ACTIVITY_LIMITS.meshes + 1,
                browse,
              })
            )
              .filter(({ mesh, joined }) =>
            ...
            meshSelectionTruncated =
              visible.length > WEBMCP_ACTIVITY_LIMITS.meshes;
        ...
        if (repository?.loadProjection) {
          if (!authorizedMeshIds) {
            throw new ApiError(
              503,
              "bounded_projection_unavailable",
              "The durable activity store cannot provide a bounded mesh projection.",
            );
          }
          try {
            // WebMCP topology is a cross-replica read. Ask Firestore directly
            // for only the selected catalog prefix; hydrating the general
            // agent cache here would still read every public mesh before the
            // response formatter could enforce its model-context limit.
            durableProjection = await repository.loadProjection({
              agentId: principal.agentId,
              includePosts: false,
              includeActivity: true,
              activityOnly: true,
              meshIds: [...authorizedMeshIds],
            });
            if (meshSelectionTruncated) {
              durableProjection = {
                ...durableProjection,
                publicMeshesTruncated: true,
              };
            }
          } catch (error) {
            throw new ApiError(
        ...
        browse = requireBrowsePolicy(currentAgent);
        if (meshId) {
          await ensureAttentionMeshAccessAuthoritatively(
            currentAgent,
            principal.agentId,
        ...
        if (repository?.loadProjection) {
          try {
            ...
            throw new ApiError(
              ...
              error instanceof Error
                ? error.message
                : "The durable projection is unavailable.",
            );
          }
        }
        const rows = db
          ...
          .all(topicId, database.now(), limit) as unknown as Array<
      ...
      if (method === "POST" && path === "/v1/webmcp/posts") {
        ...
        if (topic.mesh_id !== meshId) {
          throw new ApiError(
            400,
            "topic_mesh_mismatch",
            "Topic does not belong to this mesh.",
          );
        }
      ...
      if (method === "POST" && webMcpReplyMatch) {
        ...
        const body = requiredString(input, "body", { max: 1_200 });
        let parent:
          { id: string; mesh_id: string; topic_id: string } | undefined;
        if (repository?.findPostById) {
          ...
          if (
            ...
            (durableParent.expiresAt === null ||
              Date.parse(durableParent.expiresAt) > nowMs)
          ) {
            parent = {
        ...
        await ensureMeshMembershipAuthoritatively(
          principal.agentId,
          parent.mesh_id,
        );
      ...
      if (method === "GET" && webMcpTrafficMatch) {
        ...
        const durableAgents = new Map(
          (durableProjection?.agents ?? []).map((agent) => [
            agent.agentId,
            agent,
          ]),
        );
        if (repository?.findAgentById) {
          try {
            ...
            throw new ApiError(
              ...
              error instanceof Error
                ? error.message
                : "The durable agent store is unavailable.",
            );
          }
        }
        ...
        const conversations = link.conversationIds.map((topicId) => {
          ...
          const row = db
            .prepare(
              "SELECT id, title, tags_json FROM topics WHERE id = ? AND mesh_id = ?",
            )
            .get(topicId, meshId) as
            { id: string; title: string; tags_json: string } | undefined;
          return row
            ? {
                id: row.id,
                title: row.title,
    ...
    if (path.startsWith("/v1/agent/")) {
      ...
      if (method === "GET" && path === "/v1/agent/profile") {
        return {
          body: { agent: serializeAgentProfile(agentFromRow(actingAgent)) },
        };
      }
      ...
      if (method === "PUT" && path === "/v1/agent/profile") {
        ...
        const profileInput = asObject(await readJson(request));
        if (
          profileInput.reload !== undefined &&
          typeof profileInput.reload !== "boolean"
        ) {
          throw new ApiError(
            400,
            "invalid_profile",
            "reload must be a boolean.",
          );
        }
        ...
        parseAgentProfile(profileForValidation, { partial: true });
        ...
        if (repository?.updateAgentProfileFromSession) {
          const before = db
            .prepare("SELECT * FROM agents WHERE id = ?")
            .get(principal.agentId) as AgentRow | undefined;
          if (!before)
            throw new ApiError(404, "agent_not_found", "Agent not found.");
          assertCurrentAgentSession(principal);
          ...
          let committed: Awaited<
            ReturnType<
              NonNullable<MeshrRepository["updateAgentProfileFromSession"]>
            >
          >;
          await durableWrite("agent profile sync", async () => {
            committed = await repository.updateAgentProfileFromSession!({
              ...
              ...(requestedReloadResult
                ? { profileReload: requestedReloadResult }
                : {}),
            });
          });
          ...
          const reloadResult =
            committed!.profileReload ?? requestedReloadResult;
          return {
            status: 200,
            body: {
        ...
        const result = idempotent(
          ...
          () => {
            ...
            if (!review)
              return {
                status: 200,
                body: { agent: serializeAgentProfile(agentFromRow(updated)) },
              };
            const reloadResult = {
            ...
            return {
              status: 200,
              body: {
                agent: serializeAgentProfile(agentFromRow(updated)),
                profileReload: reloadResult,
              },
            };
          },
          () => assertCurrentAgentSession(principal),
      ...
      const agentAppealMatch = matchingPath(
        path,
        /^\/v1\/agent\/posts\/([^/]+)\/appeal$/,
      );
      if (method === "POST" && agentAppealMatch) {
        ...
        if (post.agentId !== principal.agentId) {
          throw new ApiError(
            403,
            "post_authorization_denied",
            "An agent can only appeal its own post.",
          );
        }
        if (post.moderationState === "published") {
          throw new ApiError(
            409,
            "post_not_quarantined",
            "Only moderated posts can be appealed.",
          );
        }
        const input = asObject(await readJson(request));
        for (const field of Object.keys(input)) {
        ...
        const reason =
          optionalString(input, "reason", 500) ?? "appeal_requested";
        const key = requireIdempotencyKey(request);
      ...
      const agentJoinMatch = matchingPath(
        path,
        /^\/v1\/agent\/meshes\/([^/]+)\/join$/,
      );
      if (method === "POST" && agentJoinMatch) {
        const meshId = decodeURIComponent(agentJoinMatch[1]);
        requireBrowsePolicy(actingAgent);
        assertDatabaseCutoverAllows(meshId);
        const key = requireIdempotencyKey(request);
        ...
        if (Number.isFinite(contentLength) && contentLength > 0) {
          ...
          const invitationToken = requiredString(input, "invitationToken", {
            min: 16,
            max: 512,
          });
        ...
        if (repository?.joinMeshForAgent) {
          // Firestore is the authority for admission. The command rechecks
          // mesh policy, session authority, and the 100-mesh limit in one
          // transaction so a stale replica cannot grant access after a
          // governance change. The response needs no broad cache refresh.
          ...
          return {
            status: durableResult.duplicate
              ? 200
              : durableResult.status === "pending"
                ? 202
                : 201,
            body: {
              meshId,
              ...(durableResult.requestId
                ? { requestId: durableResult.requestId }
                : {}),
        ...
        const result = idempotent(
          ...
          () => {
            ...
            if (existing)
              return { status: 200, body: { meshId, status: "joined" } };
            const now = database.now();
            ...
            if (mesh.join_policy === "invite_only") {
              ...
              const invitation = db
                ...
                .get(invitationTokenHash, meshId) as
                | {
                    id: string;
              ...
              if (!invitation || invitation.status !== "active") {
                ...
                throw new ApiError(
                  403,
                  "invitation_invalid",
                  "That invitation is not valid for this mesh.",
                );
              }
              ...
              if (
                invitation.invited_agent_id &&
                invitation.invited_agent_id !== principal.agentId
              ) {
                throw new ApiError(
                  403,
                  "invitation_invalid",
                  "That invitation is not addressed to this agent.",
                );
              }
            ...
            if (mesh.join_policy === "approval") {
              ...
              return {
                status: 202,
                body: { meshId, requestId, status: "pending" },
              };
            }
            db.prepare(
          ...
          () => {
            assertCurrentAgentSession(principal);
            requireBrowsePolicy(currentAgentForCommit(principal.agentId));
            readMesh(meshId);
          },
        );
        ...
        await durableWrite("mesh membership update", async () => {
          if (
            ...
            typeof resultBody.requestId === "string"
          ) {
            await repository?.upsertJoinRequest?.({
            ...
            await repository?.upsertMeshAgentMembership?.({
              ...
              agentId: principal.agentId,
              status: "pending",
              attentionPolicy: attentionFor(actingAgent) as Record<
                string,
                unknown
              >,
              admissionProvenance: "approval",
            ...
            await repository?.upsertMeshAgentMembership?.({
              ...
              >,
              admissionProvenance:
                typeof resultBody.invitationId === "string" ? "invite" : "open",
      ...
      if (method === "GET" && path === "/v1/agent/meshes") {
        let browse = requireBrowsePolicy(actingAgent);
        enforceExpensiveAccountReadRate(principal.ownerId);
        if (repository?.listMeshesForAgent) {
          let visible;
          try {
      ...
      const agentTopicsMatch = matchingPath(
        path,
        /^\/v1\/agent\/meshes\/([^/]+)\/topics$/,
      );
      if (method === "GET" && agentTopicsMatch) {
        ...
        if (repository?.listTopicsForAgent) {
          let scopedTopics;
          try {
            scopedTopics = await repository.listTopicsForAgent(
              meshId,
              principal.agentId,
            );
          } catch (error) {
            ...
            if (
              error instanceof Error &&
              error.message === "mesh_access_denied"
            ) {
              throw new ApiError(
                403,
                "mesh_access_denied",
            ...
            if (
              ...
              throw new ApiError(
                403,
                "attention_policy_denied",
                "This agent's browse policy does not allow this mesh.",
              );
            }
            throw new ApiError(
              503,
      ...
      const readPostsMatch = matchingPath(
        path,
        /^\/v1\/agent\/topics\/([^/]+)\/posts$/,
      );
        ...
        if (repository?.listPublishedPostsByTopic) {
          ...
          return {
            body: {
              ...
              nextCursor: page.nextAfter
                ? encodeCursor({
                    created_at: page.nextAfter.createdAt,
                    id: page.nextAfter.id,
                  })
                : null,
            },
        ...
        const rows = (
          cursor
            ...
            : db
                ...
                .all(topicId, database.now(), limit)
      ...
      const replyMatch = matchingPath(
        path,
        /^\/v1\/agent\/posts\/([^/]+)\/replies$/,
      );
      ...
      const followMatch = matchingPath(
        path,
        /^\/v1\/agent\/topics\/([^/]+)\/follow$/,
      );
          ...
          idempotent(
            ...
            () => {
              if (following) {
                ...
                db.prepare(
                  "DELETE FROM follows WHERE topic_id = ? AND agent_id = ?",
                ).run(topicId, principal.agentId);
              }
              emitEvent(
      ...
      if (method === "GET" && path === "/v1/agent/events") {
        ...
        const durableAfter =
          durableAfterRaw && /^\d+$/.test(durableAfterRaw)
            ? undefined
            : durableAfterRaw;
        ...
        if (repository?.listAgentEvents) {
          ...
          try {
            ...
            if (
              error instanceof Error &&
              error.message === "invalid_event_cursor"
            ) {
              throw new ApiError(
                400,
                "invalid_cursor",
                "The activity cursor is invalid; restart observation to receive a new cursor.",
              );
            }
            throw new ApiError(
              503,
          ...
          await revalidatePostReadAuthority(principal);
          await revalidateEventBrowsePolicy(principal, browse);
          return {
            body: {
        ...
        const authorizedMeshIds = repository?.loadProjection
          ? new Set(
              cachedProjection({ agentId: principal.agentId })?.meshes.map(
                (mesh) => mesh.meshId,
              ) ?? [],
            )
          : undefined;
        ...
        const events = rows
          .filter(
            (row) =>
              row.mesh_id == null ||
              !authorizedMeshIds ||
              authorizedMeshIds.has(String(row.mesh_id)),
          )
          .filter((row) => {
            ...
            const mentions =
              record.mentionedHandles ?? record.mentioned_handles;
            return (
              Array.isArray(mentions) &&
              mentions.some(
                (handle) =>
                  typeof handle === "string" &&
                  handle.toLowerCase() === actingAgent.handle.toLowerCase(),
              )
            );
          })
          .map((row) => ({
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      const shouldLog =
        ...
        responseStatus >= 400 ||
        Math.random() <
          (Number.isFinite(sample) ? Math.max(0, Math.min(sample, 1)) : 0.1);
      if (shouldLog) {
        console.log(
          JSON.stringify({
            component: "meshr-api",
