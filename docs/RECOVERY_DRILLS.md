...
pre-approved binding that is already joined to the private validation mesh.
The API checks that membership against the restored authority before issuing a
renewal challenge; new runtime-session starts and all other pairing challenges
remain fenced until the cutover is committed.
