...
| Firestore restore | Restore authority and topology databases into isolated named databases, authorize them with `additional_*_database_names`, switch the protected Flux values, and roll all replicas. | Private-mesh isolation, last-owner protection, session fences, topology snapshots, and the authenticated smoke all pass before the old database is retired. |
